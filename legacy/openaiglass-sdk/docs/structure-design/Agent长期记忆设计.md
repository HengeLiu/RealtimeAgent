...
3. 每轮 Agent 请求自动注入基本信息正文和个性化信息主题，但不把所有个性化信息详情都塞进上下文。
...
| 个性化信息 | 可能变化或内容较长的信息 | 每轮只注入主题，详情按需查询 | 住址、电话、爱好、习惯、任务设置 |
...
| 工具 | 作用 |
| --- | --- |
| `memory_search` | 按主题读取记忆详细内容，不负责新增、更新或删除；未命中时也会返回文本反馈。 |
...
3. 把最多 `AGENT_MEMORY_MAX_PROMPT_ITEMS` 条个性化信息主题注入 system prompt。
...
MemoryOperationPlan(
  actions=[
    MemoryOperationAction(operation="delete", topic="旧主题", memory_id="内部编号"),
    MemoryOperationAction(operation="add", memory_type="personalized", topic="新主题", content="新内容")
  ],
  feedback="已更新相关记忆"
)
```
...
1. MemoryAgent 根据 `memory_context` 和现有记忆生成 `add` 动作，决定 `memory_type`、`topic` 和 `content`。
2. `AgentMemoryRuntime.add_memory(...)` 清洗 `scope_id`、`topic` 和 `content`，主题最长保留 60 个字符，正文最长保留 4000 个字符。
3. SDK 校验作用域、主题和正文不能为空；`confidence` 会被限制在 `0.0` 到 `1.0`。
4. 创建新的 `AgentMemoryRecord`，记录内部 `memory_id`、`scope_type/scope_id`、信息类型、主题、正文、来源、创建时间和更新时间。
5. 存储层优先调用 `upsert_by_topic(...)`：同一作用域、同一信息类型、同一主题视为同一个记忆槽位。如果已有同主题记忆，则复用原 `memory_id` 和创建时间，避免重复记忆堆积。
6. `JsonFileAgentMemoryStore` 会在写入后把完整记忆列表刷新到 JSON 文件；写入采用临时文件加原子替换，降低异常退出造成半文件的概率。

新增策略的含义是：主题相同的信息默认是覆盖而不是追加。例如用户多次说“记住我的导航偏好”，SDK 会尽量维护同一条“导航偏好”记忆，而不是产生多条相互冲突的偏好。
...
| `AGENT_MEMORY_MAX_PROMPT_ITEMS` | `6` | 每轮最多注入基本信息条数和个性化信息主题条数。 |
