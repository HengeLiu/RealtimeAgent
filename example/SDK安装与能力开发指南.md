...
开发者目标不是理解本仓库内部的 `server / phone / glass` 系统实现，而是基于 SDK 提供的公开基类完成业务能力开发：
...
1. [capabilities/find_object/server/tool.py](/Users/elio/dev/llm-project/OpenAIglassesDemo_2/example/capabilities/find_object/server/tool.py)
2. [capabilities/find_object/server/task.py](/Users/elio/dev/llm-project/OpenAIglassesDemo_2/example/capabilities/find_object/server/task.py)
3. [capabilities/find_object/phone/processor.py](/Users/elio/dev/llm-project/OpenAIglassesDemo_2/example/capabilities/find_object/phone/processor.py)
4. [capabilities/find_object/phone/task.py](/Users/elio/dev/llm-project/OpenAIglassesDemo_2/example/capabilities/find_object/phone/task.py)
5. [capabilities/find_object/scenario.py](/Users/elio/dev/llm-project/OpenAIglassesDemo_2/example/capabilities/find_object/scenario.py)
6. [scenario/find_object_basic.json](/Users/elio/dev/llm-project/OpenAIglassesDemo_2/example/scenario/find_object_basic.json)

## 10. 本仓库 example 验证命令

如果开发者直接使用本仓库 example 学习，可以执行：

```bash
python script/run_sdk_package_check.py
python script/run_sdk_scenario.py --scenario-dir testdata/scenario --pretty
python script/run_sdk_preflight.py --report logs/sdk-preflight-example-dev.json
```
...
bash script/run_sdk_live_check.sh --report logs/sdk-live-check-example-dev.json
```
...
1. 修改 `server/src`。
2. 修改 `sdk/python` 内部运行时。
3. 直接读写设备绑定表。
4. 直接拼控制 WebSocket 消息。
5. 为单个能力新增专用系统接口。
6. 在根 `phone/ios` 或 `glass/src` 写具体业务逻辑。
