...
当前仓库中，根目录 `phone/ios/GlassesVideoReceiver` 应被视为通用手机 SDK运行时；`example/phone/ios/FindObjectPhoneCapability.swift` 才是官方样例业务能力。
...
5. `example/phone/ios/FindObjectPhoneCapability.swift`
6. `example/phone/ios/FindObjectPhoneCapabilityTests.swift`

当前已经形成的边界：

1. 根目录手机工程负责注册、控制消息、视频接收和通用任务承载。
2. `find_object` 业务检测逻辑已迁到 `example/phone/ios`。
...
2. 具体业务能力位于 `example/phone/ios` 或外部开发者项目。
