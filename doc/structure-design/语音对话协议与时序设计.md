...
2. 注册成功后，服务端为该设备创建或获取一条 `AgentSessionController`。
3. `AgentSessionController` 通过 `server-api` 自动向眼镜下发 `voice.session.open`，建立逻辑语音会话。
...
8. `AgentSessionController` 聚合该轮音频并调用 `qwen3-omni-flash`。
...
## 3.6 Agent-core 与 AgentSessionController

`agent-core` 和 `AgentSessionController` 的详细设计已独立到：

- [agent-core设计.md](/Users/elio/dev/llm-project/OpenAIglassesDemo_2/doc/structure-design/agent-core设计.md)

这里只保留和语音链路直接相关的结论：

- 每台眼镜一个 `AgentSessionController`
- 注册成功后自动进入语音待命
- 每次唤醒只处理一轮用户输入
- 回复播放完成后回到待命状态
...
1. `AgentSessionController` 将当前 `segment_id` 的 `audio_chunk` 聚合为完整 `wav`
...
- 回复音频数据面继续沿用旧项目下行流式通道，例如 `/stream.wav`
...
participant "AgentSessionController" as CTRL
...
- 任何失败都必须回 `task.state.changed` 或 `agent.reply.final` 风格的结构化失败结果
- 不能只打日志
- 不能只靠连接断开表达失败
...
3. 本地端点检测结束后触发模型调用
4. 模型回复继续使用流式下行播放
5. 全链路带 `trace_id`、`task_id`、`stream_id`、`segment_id`
