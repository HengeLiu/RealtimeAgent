...
本文档不要求第二期完成正式 ESP32 SDK 发布，也不要求一次性移动 `glass/src` 到 `example/glass/esp32`。
...
当前仓库中，`glass/src` 仍是实际眼镜工程；`example/glass/run.sh` 先委托该工程。第二期的收口目标是明确其 SDK运行时 职责，不要求立即完成目录迁移。
...
4. `example/glass/run.sh`
5. `example/glass/README.md`
...
4. 后续目录迁移时，再考虑把 `glass/src` 移入 `example/glass/esp32`。
