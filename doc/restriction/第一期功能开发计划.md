...
### 5 引入工具与 MCP，使大模型可以调用更多工具和服务

要求：

- 本期暂不建设独立 Skill 抽象。
- 可先实现少量简单的普通工具与 MCP 用于联调测试。

### 6 实现拍照工具，并让大模型根据图片完成解读

要求：

- 提示词示例：“请帮我看一下眼前有什么”。
- 由大模型自主调用 `capture_photo` 等拍照工具。
- 本期不要求必须封装为 Skill，也不要求单独建设图片解读 Skill 运行时。
...
### 8 实现服务器“后台任务管理”工具，并使用 LLM 创建和管理后台任务

示例：

- 用户说“帮我计时三分钟”。
- 大模型应调用后台任务创建工具，创建一个计时器后台任务。

要求：

- “后台任务管理”工具应接收后台任务类型与参数。
- 启动具体后台任务实例。
...
### 10 允许大模型通过工具创建手机与眼镜直连的后台任务

示例：

- 存在一个工具叫“手机录视频”。
- 大模型调用该工具后，眼镜与手机建立网络直连。
- 眼镜将视频流直接发送到手机。
