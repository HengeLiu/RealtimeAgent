...
本文定义 `examples/device_app_demo/ios/` 的 iOS App 目标设计。该 App 是 Swift Device SDK 的最小真机验证入口，负责页面、交互、状态展示和业务自定义事件示例；不重复实现 Swift SDK 已负责的注册、录音、播放、媒体 WebSocket、speaker buffer、AEC 或协议状态机。
...
2. App 调用 `client.startConversation(reason: "device_app_demo_start_button")`。
3. SDK 准备硬件资源：
   - 确认权限。
