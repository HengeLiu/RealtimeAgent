...
class VisionProcessor:
    ...
    async def prepare_session(self, *, purpose: str, object_name: str) -> None:
        ...
        if self.purpose == "traffic_light":
            ...
            self.find_object_detector = FindObjectDetector(self.config.find_object, device=self.config.device, object_name=self.object_name)
    ...
    async def process_frame(self, frame: bytes, *, frame_count: int) -> VisionFrameResult:
        ...
        if self.purpose == "traffic_light":
            ...
            detector = self.find_object_detector or FindObjectDetector(self.config.find_object, device=self.config.device, object_name=self.object_name)
