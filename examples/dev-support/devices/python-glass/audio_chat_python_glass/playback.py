...
def _repo_root() -> Path:
    ...
    主要逻辑：`playback.py` 位于 `examples/dev-support/devices/python-glass/audio_chat_python_glass`，
    向上三层是仓库根目录。
    返回值：仓库根目录路径。
    异常情况：无。
    """
...
class NetworkPythonPlaybackEndpoint:
    ...
    def _session_artifact_dir(self) -> Path:
        ...
        主要逻辑：RunRecorder 按 `runs_root/<user_id>/<session_id>` 保存产物。
        返回值：会话产物目录。
        异常情况：`_session_id` 为空时返回未绑定目录占位。
        """

        if not self._session_id:
            return self.runs_root / "_unbound" / "unknown"
        candidates = [
