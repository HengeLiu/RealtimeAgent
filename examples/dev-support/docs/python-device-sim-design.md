...
文档状态：目标设计文档。当前仓库仍保留 `examples/dev-support/devices/python-glass`、`examples/dev-support/devices/python-phone` 等参考端；本文描述的是后续收敛为统一无头设备模拟器的目标形态。当前可运行端侧和开发方式见 [设备注册与功能开发说明](../../../audio-server/docs/how-to/device-capability-development.md)。
...
uv run audio-chat.device.sim \
  --config examples/dev-support/devices/python-device-sim/scenarios/audio-playback.yaml
```
...
uv run python -m audio_chat_python_phone_mock --config examples/dev-support/devices/python-device-sim/scenarios/vision-task.yaml
```
