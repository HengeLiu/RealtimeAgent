...
def test_python_playback_glass_does_not_import_server_internal_runtime() -> None:
    ...
    预期结果：端侧只通过协议对话，不实例化 `AudioChatApp` 等 server 对象。
    """

    source = "\n".join(path.read_text(encoding="utf-8") for path in (DEVICE_ROOT / "audio_chat_python_playback_glass").glob("*.py"))
...
def test_pytest_integration_keeps_external_cli_boundary() -> None:
    ...
    测试方法：检查 README 中推荐 `python -m audio_chat_python_playback_glass`。
    预期结果：没有新增 `audio-chat.system-test` 命令。
    """

    readme = (DEVICE_ROOT / "README.md").read_text(encoding="utf-8")
    assert "python -m audio_chat_python_playback_glass run" in readme
    assert "audio-chat.system-test" not in readme
