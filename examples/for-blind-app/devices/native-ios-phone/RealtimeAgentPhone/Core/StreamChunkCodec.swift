...
struct RealtimeAgentStreamChunk {
    ...
    init(
        ...
        version: String = "realtime-agent.v1",
        final: Bool = false,
        metadata: [String: Any] = [:]
    ) {
        self.userID = userID
...
enum RealtimeAgentStreamChunkCodec {
    static func encode(_ chunk: RealtimeAgentStreamChunk) throws -> Data {
    ...
    static func decode(_ data: Data) throws -> RealtimeAgentStreamChunk {
        guard data.count >= 4 else {
            throw RealtimeAgentEndpointError.invalidStreamChunk("message too short")
        }
        ...
        guard headerLength > 0, headerEnd <= data.count else {
            throw RealtimeAgentEndpointError.invalidStreamChunk("invalid header length")
        }
        ...
        guard let header = try JSONSerialization.jsonObject(with: headerData) as? [String: Any] else {
            throw RealtimeAgentEndpointError.invalidStreamChunk("header is not json object")
        }
        let payloadSize = header["payload_size"] as? Int ?? -1
        guard payloadSize == payload.count else {
            throw RealtimeAgentEndpointError.invalidStreamChunk("payload_size mismatch")
        }
        return RealtimeAgentStreamChunk(
            ...
            version: header["version"] as? String ?? "realtime-agent.v1",
    ...
    private static func string(_ header: [String: Any], _ key: String) throws -> String {
        guard let value = header[key] as? String else {
            throw RealtimeAgentEndpointError.invalidStreamChunk("missing \(key)")
        }
        return value
    }
