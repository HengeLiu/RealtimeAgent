...
struct RealtimeAgentEvent {
    ...
    init(dictionary: [String: Any]) throws {
        guard let eventName = dictionary["event_name"] as? String,
            ...
            throw RealtimeAgentEndpointError.invalidEvent("missing event_name/user_id/producer_id")
        }
        self.eventName = eventName
        self.userID = userID
        self.producerID = producerID
        self.payload = dictionary["payload"] as? [String: Any] ?? [:]
        self.version = dictionary["version"] as? String ?? "realtime-agent.v1"
...
/// realtime-agent id 和时间工具。
enum RealtimeAgentIDs {
...
/// iOS 参考端协议错误。
enum RealtimeAgentEndpointError: LocalizedError {
