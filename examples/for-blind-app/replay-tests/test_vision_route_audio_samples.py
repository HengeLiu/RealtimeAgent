...
from realtime_agent import AssetRef
from realtime_agent.app import RealtimeAgentApp, RealtimeAgentConfig
from realtime_agent.server import RealtimeAgentHttpServer
from realtime_agent.tools import ToolExecutor
from realtime_agent_python_glass.playback import NetworkPythonPlaybackEndpoint, PythonPlaybackEndpoint, load_wav_audio
...
def _build_for_blind_text_app(tmp_path: Path, monkeypatch) -> RealtimeAgentApp:
    """创建加载 for-blind-app 能力的 Vision 路线应用。

    测试目标：让应用级 `capture_photo` Tool 通过真实自动发现进入注册表。
    测试方法：读取 for-blind-app 的 server.yaml，并把运行产物路径替换到临时目录。
    预期结果：Vision 路线可调用应用级视觉 Tool，而 SDK 默认应用不暴露该 Tool。
    """
...
@pytest.mark.replay
def test_vision_route_uses_audio_sample_filename_as_mock_asr_transcript_and_calls_device_tool(tmp_path: Path) -> None:
    """测试目标：验证 Vision 路线可以用真实 AudioSample 音频完成自动化回放。

    测试方法：Python glass 无头端测上传当前 WAV 样例；mock ASR 根据 WAV 文件名
    生成转写文本，mock vision model 按转写触发 `query_device_state` 工具。
    预期结果：ASR、VisionRealtimeAgentCore、ToolGateway、流式 TTS 和 speaker 输出全部产生
    可回放产物，工具轨迹中包含设备状态查询。
    """

    app = RealtimeAgentApp(RealtimeAgentConfig(runs_root=str(tmp_path / "runs"), agent_mode="vision"))
    endpoint = PythonPlaybackEndpoint(app=app, user_id="user-vision-sample", device_id="dev-vision-sample")
...
@pytest.mark.replay
def test_vision_route_capture_photo_tool_collects_rgb_asset_from_python_glass(tmp_path: Path, monkeypatch) -> None:
    """测试目标：验证 Vision 路线中的视觉工具仍遵守 stream/event 协议。

    测试方法：使用“看一下我前面有什么”音频样例触发 `capture_photo`；Python glass 收到
    `stream.control.open.requested` 后上传一帧 `sensor.rgb` 资产。
    预期结果：工具调用成功，资产缓存写入照片，最终Vision 回复来自工具结果而不是固定图片。
    """

    app = _build_for_blind_text_app(tmp_path, monkeypatch)
    endpoint = PythonPlaybackEndpoint(app=app, user_id="user-vision-photo", device_id="dev-vision-photo")
...
@pytest.mark.replay
def test_vision_route_network_python_glass_replays_audio_sample_over_websocket(tmp_path: Path) -> None:
    """测试目标：验证 Vision 路线可以通过真实 HTTP/WebSocket 端测完成无头自动化。
    ...
    async def run() -> None:
        ...
        try:
            endpoint = NetworkPythonPlaybackEndpoint(
                ...
                user_id="user-vision-network",
                device_id="dev-vision-network",
                runs_root=str(tmp_path / "runs"),
            )
            audio = load_wav_audio(AUDIO_SAMPLE_ROOT / "帮我查一下我眼镜的状态.wav")
        ...
        assert result["input_audio"]["chunk_count"] > 1
        assert (tmp_path / "runs" / "user-vision-network" / result["session_id"] / "events.jsonl").exists()
        assert (tmp_path / "runs" / "user-vision-network" / result["session_id"] / "model-request.json").exists()
