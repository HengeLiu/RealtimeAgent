...
def test_esp32_reference_firmware_manifest_is_complete() -> None:
    ...
    测试方法：检查 `examples/for-blind-app/devices/native-esp32-glass/firmware` 下 ESP-IDF 必要文件和组件依赖。
    预期结果：CMake、sdkconfig defaults、component manifest 和参考 main 文件都存在，
    且 manifest 声明 WebSocket 与 JSON 依赖。
    """

    result = _esp32_project_manifest_check(AUDIO_ROOT / "examples/for-blind-app/devices/native-esp32-glass/firmware")
...
def test_esp32_build_dry_run_reports_command_with_reference_project() -> None:
    ...
    assert "examples/for-blind-app/devices/native-esp32-glass/firmware" in completed.stdout
