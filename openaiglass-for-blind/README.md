...
跨设备启动顺序：先用 `openaiglass.server.start --app-module host.server.main --app-root openaiglass-for-blind` 启动服务端，再用 `openaiglass.phone.open --app-root openaiglass-for-blind` 启动手机端入口，最后用 `openaiglass.glass.start --repo-root .` 启动或烧录眼镜端工程；设备级回放时用 `openaiglass.glass.start --runtime playback --config <config>` 启动虚拟眼镜。
