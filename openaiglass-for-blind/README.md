# openaiglass-for-blind

本目录维护“盲人 AI 眼镜”真实场景工程。它基于 [openaiglass-sdk](../openaiglass-sdk) 提供的三端开发框架实现业务能力，不重复实现 SDK 的通信、运行时、日志、异常和上下文底座。

## 目录职责

| 目录 | 职责 |
| --- | --- |
| [docs](./docs) | 产品需求、功能设计、阶段计划、实施记录、验收和迁移文档。 |
| [host](./host) | 盲人产品宿主，包含很薄的服务端、手机端和眼镜端入口。 |
| [capabilities](./capabilities) | 真实盲人场景业务能力实现，例如找物体能力。 |
| [testdata](./testdata) | 设备级回放使用的音频、视频、图像和传感器数据资产。 |
| [SDK安装与能力开发指南.md](./SDK安装与能力开发指南.md) | 给业务开发者看的 SDK 安装、能力扩展和联调说明。 |

## 开发者入口

1. 先读 [SDK安装与能力开发指南.md](./SDK安装与能力开发指南.md)，理解如何基于 SDK 扩展能力。
2. 再看 [capabilities/find_object](./capabilities/find_object)，这是当前真实业务能力实现。
3. 进入真机联调前执行：

```bash
uv run openaiglass.sdk.preflight --report logs/sdk-preflight-current.json
uv run openaiglass.config.sync --app-root openaiglass-for-blind
```

跨设备启动顺序：先用 `uv run openaiglass.server.run --app-module host.server.main --app-root openaiglass-for-blind` 启动服务端，再用 `uv run openaiglass.phone.open --app-root openaiglass-for-blind` 启动手机端入口，最后用 `uv run openaiglass.glass.start --repo-root .` 启动或烧录眼镜端工程；设备级回放时用 `uv run openaiglass.glass.start --runtime playback --config <config>` 启动虚拟眼镜。
