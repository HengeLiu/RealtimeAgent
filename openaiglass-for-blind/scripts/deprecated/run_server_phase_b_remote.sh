...
REMOTE_LOG_FILE="${REMOTE_LOG_FILE:-${REMOTE_LOG_DIR}/phase_b_server.log}"
REMOTE_PID_FILE="${REMOTE_PID_FILE:-${REMOTE_LOG_DIR}/phase_b_server.pid}"
HOST="${HOST:-0.0.0.0}"
PORT="${PORT:-8765}"
...
Actions:
  sync   上传本地代码到远程服务器
  start  远程补依赖并启动 Phase B 服务端
  stop   停止远程 Phase B 服务端
  logs   跟随远程日志
  all    sync + start + logs
...
start_remote() {
  echo "[start] Starting Phase B server on ${REMOTE_HOST}"
  ssh "${REMOTE_HOST}" \
...
chmod +x script/run_server_phase_b.sh
...
bash script/run_server_phase_b.sh start
EOF
}

stop_remote() {
  echo "[stop] Stopping Phase B server on ${REMOTE_HOST}"
  ssh "${REMOTE_HOST}" \
...
cd "${REMOTE_DIR}"
chmod +x script/run_server_phase_b.sh
LOG_DIR="${REMOTE_LOG_DIR}" LOG_FILE="${REMOTE_LOG_FILE}" PID_FILE="${REMOTE_PID_FILE}" bash script/run_server_phase_b.sh stop
EOF
}
