...
当前指南对应 SDK 版本：`sdk-v90`。本版本修复 Omni Realtime 下行音频流收口事件处理：SDK 会把官方 `response.audio.done` 作为音频输出完成信号，不再只等待 `response.done`，避免模型已经完成音频和转写但播放 HTTP 流迟迟不结束，导致眼镜端卡在播放状态并影响连续追问。公网/NAT 穿透、跨机器分布式任务平台、iOS 二进制 XCFramework 和 ESP32 component registry 发布暂不覆盖。
...
Omni Realtime 模式下可观察这些日志：`Omni Realtime 预连接已建立`、`Omni Realtime 首段上行音频已推送`、`Omni Realtime 请求已提交`、`Omni semantic_vad 等待自动响应`、`Omni Realtime 返回首个文本`、`Omni Realtime 返回首段音频`、`Omni Realtime 音频输出完成`、`Omni Realtime 最终回复`。`sdk-v90` 起，SDK 以 `response.audio.done` 作为下行音频流完成信号，并继续兼容 `response.done` / `response.cancelled` 作为响应对象终态；`response.audio_transcript.done` 只用于助手文本完成记录，不再被当作播放流完成依据。`VOICE_OMNI_PHOTO_WAIT_MS` 控制服务端等待本轮自动照片上传完成的时间，默认 `300` 毫秒；等待失败会继续走纯语音输入，不阻塞主链路。
