...
当前指南对应 SDK 版本：`sdk-v63`。本版本继续推进“方案二”Omni 语义实时连续对话：默认 `VOICE_CONVERSATION_MODE=realtime_semantic_vad`，服务端在语音开始时前置自动抓拍并异步追加到 Omni，由 Omni `semantic_vad` 自动提交和自动响应；真实 `glass-esp32` 在一次 WakeNet 命中后会进入短时间连续对话窗口，后续可由本地 VAD 触发下一段语音，不再强制每轮重复唤醒词。`sdk-v56` 在 ESP32-S3 固件中默认启用 AEC 试验链路，播放期间可继续保持连续对话监听；`sdk-v57` 在首次 WakeNet 唤醒成功后增加本地轻提示音；`sdk-v58` 修复 AEC 能力早报、首轮本地 segment 被播放声拖到超时、以及连续对话多轮 `stream_id` 问题；`sdk-v59` 修复插话后旧 Omni 回复迟到写回，并把 Omni `semantic_vad` 未自动提交的兜底从原会话手动提交改为 `segment_turn` 重连；`sdk-v60` 给 ESP32 播放中插话增加播放起始保护窗和连续语音帧确认，避免播放刚开始的回声被误判为插话；`sdk-v61` 进一步把播放中打断判定从端侧本地 VAD 改为 Omni `semantic_vad` 确认：端侧只启动候选语音段，服务端只有收到 Omni `input_audio_buffer.speech_started` 后才中断旧播放，未被 Omni 确认、或没有真正中断旧播放的候选段按回声丢弃；`sdk-v62` 让 ESP32 在 `sensor.audio.segment.started` 中显式带上 `started_during_playback` 和 `playback_stream_id`，避免服务端本地播放状态提前清理时误把回声候选当作普通语音；`sdk-v63` 在 Omni 流式上行 WebSocket 已关闭时跳过 `segment_turn` 重连兜底，避免上游容量限制下继续创建新连接。公网/NAT 穿透、跨机器分布式任务平台、iOS 二进制 XCFramework 和 ESP32 component registry 发布暂不覆盖。
...
ESP32-S3 播放中自然插话由固件 `sdkconfig` 控制，默认值在 `openaiglass-sdk/glass-esp32/sdkconfig.defaults` 中维护：`CONFIG_GLASS_ENABLE_AEC=y` 打开 ESP-SR AFE AEC；`CONFIG_GLASS_AEC_REFERENCE_BUFFER_MS=1200` 控制播放参考环形缓冲时长；`CONFIG_GLASS_BARGE_IN_GRACE_MS=900` 控制播放首段音频写入扬声器后的插话保护窗；`CONFIG_GLASS_BARGE_IN_MIN_SPEECH_FRAMES=6` 控制保护窗结束后需要连续命中的 VAD 语音帧数；`CONFIG_SPIRAM_ALLOW_STACK_EXTERNAL_MEMORY=y` 允许播放任务栈放到 PSRAM。首次 WakeNet 唤醒提示音由 `CONFIG_GLASS_WAKE_PROMPT_TONE_ENABLE=y` 控制，可通过 `CONFIG_GLASS_WAKE_PROMPT_TONE_DURATION_MS`、`CONFIG_GLASS_WAKE_PROMPT_TONE_FREQ_HZ`、`CONFIG_GLASS_WAKE_PROMPT_TONE_GAIN_PERMILLE` 调整时长、频率和音量。真机内存不足、扬声器参考信号不同步或误触发严重时，可先增大 `CONFIG_GLASS_BARGE_IN_GRACE_MS` 或 `CONFIG_GLASS_BARGE_IN_MIN_SPEECH_FRAMES`；`sdk-v61` 起这些阈值只影响候选段启动，真正打断还需要 Omni `semantic_vad` 确认；`sdk-v62` 起端侧会把播放中的候选段显式标记到控制消息中，服务端不会再仅依赖本地播放状态推断。仍不稳定时关闭 AEC 回退半双工，再继续调声学参数。已有旧 `sdkconfig` 时，重新烧录前要执行 `idf.py reconfigure` 或删除旧 `sdkconfig` 后重新配置，确保这些默认项进入实际构建。
...
`sdk-v58` 起，真实 ESP32-S3 在 AFE 初始化完成后会补发一次 `voice.realtime.session.opened`，避免服务端在 AEC 尚未 ready 时误判为 `half_duplex`。服务端开始下行回复时，固件会用 `finish_reason=server_response_started` 提前关闭当前本地语音段，防止播放声让本地 VAD 持续占住首轮 segment；连续对话每轮都会生成新的上行 `stream_id`。

`sdk-v59` 起，播放中插话打断后，服务端会把旧 Omni 回复的迟到音频和最终 `assistant.reply` 一并丢弃，不再把旧回复写入对话上下文。若新一轮 Omni `semantic_vad` 没有自动提交，服务端会改用 `segment_turn` 重连兜底，避免第二轮等待 45 秒超时或触发上游 1011。

`sdk-v61` 起，ESP32-S3 播放中本地 VAD 不再直接发送 `user.voice.interrupt`。固件会等首段播放 PCM 写入扬声器后经过 `CONFIG_GLASS_BARGE_IN_GRACE_MS` 保护窗，并且后续连续命中 `CONFIG_GLASS_BARGE_IN_MIN_SPEECH_FRAMES` 个语音帧后，只启动一个“候选语音段”；服务端把候选音频送入 Omni Realtime，只有收到 Omni `input_audio_buffer.speech_started` 后才真正中断旧播放。若候选段结束时 Omni 没有自动提交，或 Omni 虽然返回了语音事件但服务端没有实际中断旧播放，服务端都会按播放回声候选丢弃，不再用 `segment_turn` 兜底生成新回复，避免“播一点就停，然后循环”。`sdk-v62` 起，ESP32 会在 `sensor.audio.segment.started` payload 中携带 `started_during_playback=true` 和 `playback_stream_id`，便于服务端稳定识别播放中候选段。`sdk-v63` 起，如果 Omni 预连接已经在用户说话过程中关闭，服务端不会再为同一段音频立刻创建 `segment_turn` 兜底连接，避免在百炼返回 `Too many requests` 时进入重连放大。
