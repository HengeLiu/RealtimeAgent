...
当前指南对应 SDK 版本：`sdk-v97`。本版本开始按 Omni Server / Text Server 模态隔离设计落地第一阶段，新增 `voice.server_mode` / `VOICE_SERVER_MODE`，旧 `voice.reply_mode` / `VOICE_REPLY_MODE` 仍兼容映射。公网/NAT 穿透、跨机器分布式任务平台、iOS 二进制 XCFramework 和 ESP32 component registry 发布暂不覆盖。
