...
当前指南对应 SDK 版本：`sdk-v91`。本版本补强 Omni Realtime 真机排障和播放流收口：服务端会在 DEBUG 日志中打印 Omni server event 摘要，并在收到 `response.audio.done` 后先 finalize 下行播放流，再后台关闭 DashScope Realtime 会话，避免底层 SDK `close()` 阻塞到服务端超时导致眼镜端继续卡在播放流读取中。公网/NAT 穿透、跨机器分布式任务平台、iOS 二进制 XCFramework 和 ESP32 component registry 发布暂不覆盖。
