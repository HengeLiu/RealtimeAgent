...
当前指南对应 SDK 版本：`sdk-v87`。本版本在 `sdk-v86` 受限连续对话基础上，增加语音轮次意图裁决：非视觉问题不再自动抓拍，短误触发、空转写、语气词和疑似助手回声不会进入 Omni/Agent 回复链路。公网/NAT 穿透、跨机器分布式任务平台、iOS 二进制 XCFramework 和 ESP32 component registry 发布暂不覆盖。
