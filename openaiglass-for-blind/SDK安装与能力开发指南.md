...
当前指南对应 SDK 版本：`sdk-v104`。本版本在 `sdk-v103` 的通知语音桥接拆分基础上，继续把 Omni Realtime 工具桥迁入 `runtime.omni.tool_bridge`，把 Text 链路 Agent Turn 构造迁入 `runtime.text.text_agent_adapter`。业务侧使用方式不变，旧 `voice.reply_mode` / `VOICE_REPLY_MODE` 仍兼容映射。公网/NAT 穿透、跨机器分布式任务平台、iOS 二进制 XCFramework 和 ESP32 component registry 发布暂不覆盖。
