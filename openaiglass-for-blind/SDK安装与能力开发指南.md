...
当前指南对应 SDK 版本：`sdk-v68`。本版本在 `sdk-v67` 的长期记忆分类语义基础上，补强主 Agent 主动记忆提示：用户自然说出姓名、称呼、导航偏好等值得长期保存的信息时，即使没有说“记住”，主 Agent 也应调用 `manage_memory`。公网/NAT 穿透、跨机器分布式任务平台、iOS 二进制 XCFramework 和 ESP32 component registry 发布暂不覆盖。
