...
当前指南对应 SDK 版本：`sdk-v103`。本版本在 `sdk-v102` 的进度播报缓存拆分基础上，继续把通知与 Task 事件到语音播放的桥接逻辑从 `voice_runtime.py` 拆到独立模块。业务侧使用方式不变，旧 `voice.reply_mode` / `VOICE_REPLY_MODE` 仍兼容映射。公网/NAT 穿透、跨机器分布式任务平台、iOS 二进制 XCFramework 和 ESP32 component registry 发布暂不覆盖。
