...
当前指南对应 SDK 版本：`sdk-v98`。本版本在 `sdk-v97` 的 `voice.server_mode` 配置基础上，新增 Omni/Text Server 适配器和 TextDialogStateMachine，继续推进 Omni Server / Text Server 模态隔离。旧 `voice.reply_mode` / `VOICE_REPLY_MODE` 仍兼容映射。公网/NAT 穿透、跨机器分布式任务平台、iOS 二进制 XCFramework 和 ESP32 component registry 发布暂不覆盖。
