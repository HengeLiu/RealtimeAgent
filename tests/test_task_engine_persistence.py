...
def test_jsonl_task_store_restores_unfinished_task(tmp_path) -> None:
    ...
    测试方法：用 JsonlTaskStore 创建 running 任务，再用新的 store/engine 重放。
    ...
    assert second.query(ref.task_id).state == "running"
...
def test_jsonl_task_store_keeps_terminal_state_after_restart(tmp_path) -> None:
    ...
    预期结果：重放后状态仍为 completed，`restore_unfinished()` 不返回该任务。
    """
    ...
    assert second.query(ref.task_id).state == "completed"
