...
def test_old_sdk_parity_provider_lane_is_registered() -> None:
    ...
    `device-api-upgrade-provider` 覆盖 provider、MCP、preflight 和真实 provider 集成测试。
    ...
    assert "device-api-upgrade-provider" in module.CHECKS
    command_text = "\n".join(" ".join(command.command) for command in module.CHECKS["device-api-upgrade-provider"])
