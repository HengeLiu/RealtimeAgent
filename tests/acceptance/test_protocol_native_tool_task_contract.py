...
def test_user_device_context_exposes_protocol_native_api_only(tmp_path) -> None:
    ...
    context = ToolDeviceFacade(user_id="user-api", app=app)
...
def test_publish_event_broadcasts_by_subscription_not_device_id(tmp_path) -> None:
    ...
    result = ToolDeviceFacade(user_id="user-broadcast", app=app).publish_event(
    ...
    assert [event.event_name for event in first.events] == ["stream.control.open.requested"]
    assert [event.event_name for event in second.events] == ["stream.control.open.requested"]
    assert other.events == []
...
def test_payload_only_control_event_does_not_open_stream(tmp_path) -> None:
    ...
    result = ToolDeviceFacade(user_id="user-control", app=app).publish_event(
        "command.requested",
        payload={
