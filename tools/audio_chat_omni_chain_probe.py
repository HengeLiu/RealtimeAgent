#!/usr/bin/env python3
"""audio-chat Omni 链路探针。

主要功能：
1. 使用当前 `audio_chat.agent_core.realtime.QwenOmniRealtimeAdapter`，不绕过主链路 adapter。
...
示例：
    uv run python tools/audio_chat_omni_chain_probe.py \
      ...
      --out runs/omni-realtime-probe/audio-chat-chain-events.jsonl
"""
...
def _capture_photo_schema(*, flat: bool) -> dict[str, Any]:
    ...
    `flat=True` 使用当前 audio-chat adapter 实际传给 Qwen 的扁平结构；
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="测试当前 audio-chat Qwen Omni adapter 的音频-工具-图片链路。")
    ...
    parser.add_argument("--out", type=Path, default=Path("runs/omni-realtime-probe/audio-chat-chain-events.jsonl"))
...
def main() -> int:
    """运行当前 audio-chat adapter 的真实链路探针。"""
    ...
    def on_tool_done(record: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "message": "已完成一次抓拍。",
            "error": None,
            "meta": {"probe": "audio_chat_omni_chain"},
        }
        ...
        for index, chunk in enumerate(send_chunks):
            ...
            adapter.append_audio(
                StreamChunk(
                    ...
                    metadata={"probe": "audio_chat_omni_chain"},
                )
            )
