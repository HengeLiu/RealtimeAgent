...
def test_input_stream_sequence_accepts_open_upload_close_lifecycle() -> None:
    ...
    测试方法：校验 start requested -> input opened -> input closed 事件序列。
...
def test_input_stream_sequence_rejects_close_before_opened() -> None:
    ...
    测试方法：构造 start requested 后直接 closed 的反例序列。
...
def test_output_stream_sequence_accepts_playback_lifecycle() -> None:
    ...
    测试方法：校验 start requested -> ready -> started -> finished 事件序列。
...
def test_output_stream_sequence_accepts_cancel_lifecycle() -> None:
    ...
    测试方法：校验 start requested -> ready -> started -> cancel requested -> cancelled 事件序列。
...
def test_output_stream_sequence_accepts_ready_then_finished_without_started() -> None:
    """测试目标：确认空输出或未达到起播水位的输出可以在 ready 后直接完成。

    测试方法：校验 start requested -> ready -> finished 事件序列。
...
def test_output_stream_sequence_rejects_finish_before_started() -> None:
    ...
    测试方法：构造 start requested 后直接 finished 的反例序列。
    预期结果：状态机抛出 ValueError，暴露端侧播放状态回报错误。
    """

    with pytest.raises(ValueError, match="finished must appear after"):
        validate_output_stream_event_sequence(["stream.output.start.requested", "stream.output.finished"])
