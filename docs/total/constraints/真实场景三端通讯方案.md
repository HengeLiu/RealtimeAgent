...
- 断链上报与一次自动恢复参考实现
- 服务器侧语音对话会话管理
- 当前主流程：单次录音文件 -> ASR -> 对话 -> TTS 流式下发
- 预留扩展：实时音频块 -> 实时 ASR -> 中断式回复 -> TTS 流式下发
...
10. 语音对话当前主链路是“开始录音 / 结束录音并发送”的单次录音模式；实时对话能力仅保留为后续扩展。
...
- 当前推荐联调方式：独立启动 `server`、`glass`，再在 iPhone 真机运行 `clients/phone_flutter`
...
- 当前单次录音模式下，眼镜在上传音频文件后建立语音 WebSocket，服务器向眼镜流式下发 TTS 音频
- 后续若启用实时对话模式，眼镜再通过同一类 WebSocket 向服务器持续上传麦克风 PCM 音频块
- 后续若启用实时对话模式，服务器向眼镜流式下发 TTS 音频
