...
- 手机控制面参考实现
  - [app.py](/Users/elio/dev/llm-project/OpenAIglasses_for_Navigation/nextgen/apps/phone/runtime/app.py)
  - [http_control_app.py](/Users/elio/dev/llm-project/OpenAIglasses_for_Navigation/nextgen/apps/phone/runtime/http_control_app.py)
...
- 简单 Web 状态页
- 真实三进程控制面启动与回调编排
- 通过 `run_real_control_plane_demo.py` 跑通一次完整参考链路
- 任务级 WebSocket 数据面参考实现
- 手机通过 WebSocket 直接向眼镜回找物引导建议
- 手机只向服务器回任务状态、眼镜向服务器回执行状态
- 断链上报与一次自动恢复参考实现
- 通过 `run_real_find_object_demo.py` 跑通找物通讯链路
- 通过 `run_real_peer_link_recovery_demo.py` 跑通一次断链恢复链路
- 服务器侧语音对话会话管理
...
- 手机控制面：
  - `uv run python scripts/run_phone_control_runtime.py --host 127.0.0.1 --port 18092 --advertise-host 127.0.0.1 --server-base-url http://127.0.0.1:18090`
- 一键联调演示：
  - `uv run python scripts/run_real_control_plane_demo.py`
- 一键找物通讯演示：
  - `uv run python scripts/run_real_find_object_demo.py`
- 一键断链恢复演示：
  - `uv run python scripts/run_real_peer_link_recovery_demo.py`

### 1.3 当前已验证结果

截至 2026-04-03，本仓库已在本地成功验证以下链路：
...
5. 眼镜可根据服务器下发参数完成任务级 peer-link 连接动作。
...
- 运行方式：`uv run python scripts/run_real_control_plane_demo.py`
- 找物引导文本示例：`已发现手机，位置：right`
- 找物通讯运行方式：`uv run python scripts/run_real_find_object_demo.py`
- 断链恢复运行方式：`uv run python scripts/run_real_peer_link_recovery_demo.py`
...
1. 眼镜端和手机端先统一用 Python 参考实现，后续再翻译成对应语言。
