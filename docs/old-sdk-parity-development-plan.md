...
本阶段完成后，`audio-chat` 应能作为新项目优先使用的 SDK。旧实现归入 `legacy`，新能力开发应能在根目录的新 SDK 结构中找到等价路径。
...
5. 不要求把 iOS / ESP32 正式端侧工程放入 Python SDK 包内，`device-examples` 只提供参考端侧样例和契约。
...
```text
audio-chat-sdk/audio_chat/__init__.py
audio-chat-sdk/audio_chat/tools.py
audio-chat-sdk/audio_chat/tasks.py
audio-chat-sdk/audio_chat/context.py
...
audio-chat-sdk/audio_chat/cli/esp32.py
docs/esp32-s3-endpoint-bridge.md
tests/test_esp32_config_sync.py
tests/test_esp32_package_manifest.py
tests/acceptance/test_esp32_s3_endpoint_contract.py
```
...
```text
audio-chat-sdk/audio_chat/agent_core/providers.py
audio-chat-sdk/audio_chat/output/service.py
