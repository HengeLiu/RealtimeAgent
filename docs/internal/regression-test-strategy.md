...
当前状态：设计草案和历史阶段记录。本文档用于统一 `realtime-agent` 后续自动化回归测试的分层、范围、命令和测试替身边界；其中较早阶段记录里提到的 Python / TypeScript / Kotlin Device SDK 路径是历史计划或旧实现快照，不代表当前仓库现状。

当前公开测试入口以 [测试体系说明](../testing.md) 为准：Device SDK 现状按 JavaScript、Swift、C 三条路径维护；ESP32-S3 作为 C Device SDK 的端侧参考工程，通过固件构建和真机联调验证。
