...
核心目标是：先保证标准通讯协议稳定，再用系统级集成测试保证 Server SDK 和 Device SDK 对协议输入的响应稳定，然后验证真实大模型接入，最后再测试具体服务器应用功能和端侧应用功能。
...
| 层级 | 主要问题 |
| --- | --- |
| L0 协议层 | server 和 device 是否仍遵守同一套通讯契约。 |
| L1 SDK 层 | Server SDK / Device SDK 面对协议输入时，是否产生符合预期的事件响应、状态变化、处理流程和开发者 API 行为。 |
...
协议层测试只验证通讯契约，不验证模型、不验证业务、不验证真实设备。
...
3. 设备能力声明稳定。
4. 命令请求和回执稳定。
5. 输入 / 输出 stream 生命周期稳定。
6. 二进制 stream chunk 编解码稳定。
7. 错误码和错误 payload 稳定。
8. 多语言 SDK 可复用同一批 golden fixtures。

### 3.2 测试内容

| 类型 | 测试内容 |
| --- | --- |
| 事件信封 | `version`、`event_id`、`event_name`、`timestamp_ms`、`user_id`、`producer_id`、`payload`。 |
| 注册协议 | `control.device.register.requested`、结构化 `supports`、禁止旧字段。 |
| 设备能力 | `supports.sensors`、`supports.actuators`、系统音频属性边界。 |
| 命令协议 | `command.requested`、`accepted`、`progress`、`completed`、`failed`。 |
| stream 控制 | `stream.control.open.requested`、`stream.control.close.requested`。 |
| 输出 stream | `stream.output.open.requested`、`started`、`finished`、`cancelled`、`failed`。 |
| 二进制帧 | header length、JSON header、payload size、seq、final、codec。 |
| 错误码 | 标准错误码、错误 message、可诊断 metadata。 |
| 反例 | 缺字段、错字段、未知事件、旧字段、payload_size 不一致。 |
...
协议层必须 100% 自动化，且不依赖网络、API Key、真实模型或真实设备。
...
1. 所有旧 golden fixtures 仍可解析。
2. 事件名 enum 没有意外漂移。
3. stream chunk codec 与 golden binary fixture 兼容。
4. 旧字段不会被重新接受。
5. 媒体 bytes 不会进入控制事件 payload。
...
2. 正例 fixture。
3. 反例 fixture。
4. server runtime 解析测试。
5. 至少一个 Device SDK 解析测试。
...
SDK 层测试是系统级集成测试，不是方法级单元测试。它验证 Server SDK 和 Device SDK 在面对标准协议输入时，是否能产生符合预期的事件响应、状态变化、处理流程、开发者 API 行为和运行产物。

SDK 层不测试具体 app 业务，也不依赖真实模型、真实摄像头、真实手机或真实硬件。
...
1. L0 补协议 schema 和 fixture。
2. L1 Server SDK 补 `context.devices.sensors.tof.one()/stream()`。
3. L1 Device SDK 补 `DeviceBuilder.sensor_tof()` 和 handler。
...
| 变更类型 | 必跑测试 |
| --- | --- |
| 修改协议 schema / event / stream codec | L0 + 相关 Device SDK contract。 |
| 修改 Server SDK control / stream / context | L1 Server SDK 协议输入集成测试 + L1 interop。 |
| 修改 Device SDK | L0 + 对应语言 SDK + L1 interop。 |
...
| 修改 browser / phone / iOS / ESP32 端侧参考工程 | L0/L1 device contract + L3 端侧联调。 |
...
markers = [
    "protocol: L0 protocol contract tests",
    "sdk: L1 SDK tests",
    "device_sdk: L1 Device SDK tests",
...
本计划按“协议先行、再测 SDK、再测模型、最后测应用”的顺序推进。协议不是一个抽象口号，必须先落成可阅读、可追踪、可变更、可测试的规范资产；后续 L0/L1/L2/L3 都以这份协议规范为基础。
...
| Phase 2 | L0 协议层 | 把标准通讯协议测试补成独立、快速、强约束的回归入口。 |
| Phase 3 | L1 Device SDK | 让各语言 Device SDK 用协议事件和 stream 输入做系统级 contract。 |
| Phase 4 | L1 Server SDK | 标准化 Server SDK 的协议输入集成测试和 fake provider。 |
...
| 字段 | 说明 |
| --- | --- |
| `layer` | `protocol_spec`、`L0_protocol`、`L1_server_sdk` 等。 |
| `command` | 实际执行命令。 |
...
1. 先更新 `docs/reference/protocol.md` 的协议语义。
2. 同步更新 schema / AsyncAPI / error codes。
3. 更新 `testdata/protocol` 正例和反例 fixtures。
...
```bash
uv run python -m pytest audio-server/tests/acceptance/test_protocol_document_contract.py -q
```

完成标准：

1. `docs/reference/protocol.md` 成为协议正式入口。
2. 协议文档能反向定位到代码实现、schema 和 fixtures。
3. 任何协议变更都有明确 checklist。
4. L0 测试只从这套协议资产派生，不再依赖口头约定。
...
1. 把事件 schema、设备 schema、stream schema 和错误码测试集中到协议层入口。
2. 扩充 `testdata/protocol` golden fixtures。
3. 增加协议反例测试，例如旧字段、缺字段、未知事件名、非法 stream header。
4. 增加协议状态机测试，例如 command 和 stream 生命周期非法跳转。

主要产物：

```text
audio-server/tests/protocol/
testdata/protocol/
...
1. L0 不依赖网络、API Key、真实模型或真实设备。
2. 所有 protocol golden fixtures 都被至少一个测试消费。
3. 协议变更必须先更新 schema 和 fixture，否则 L0 失败。
4. stream codec 和 event envelope 有正例和反例测试。
5. L0 报告列出本次消费的 schema、fixture 数量、正例数量和反例数量。
...
1. Python / TypeScript / Swift / Kotlin / C Device SDK 逐步消费同一批 `testdata/protocol`。
...
```text
audio-device/python/tests/
audio-device/typescript/tests/
audio-device/swift/Tests/
audio-device/kotlin/device/src/test/
audio-device/c/tests/
...
cd audio-device/c && cmake -S . -B build && cmake --build build && ctest --test-dir build
```
...
examples/for-blind-app/tests/replay/
examples/for-blind-app/tests/acceptance/
examples/dev-support/tests/
...
当前仓库已经完成 L0 / L1 / L2 / L3 的首轮自动化落地。核心现状如下：

1. L0 协议层已有正式协议文档、schema / fixture / 状态机测试和反例 fixture 消费测试。
...
2. 继续抽取历史内联 fake provider，形成 `audio-server/tests/provider_contracts/` 级别的统一 contract。
...
- 实现：
  - 新增 `docs/reference/protocol.md`，作为协议正式入口。
  - 文档覆盖协议目标、协议版本、控制通道、stream 通道、事件信封、设备注册、能力声明、命令生命周期、输入 / 输出 stream 生命周期、stream 二进制帧、错误码、代码映射、协议资产、协议变更流程和兼容性策略。
  - 新增 `audio-server/tests/acceptance/test_protocol_document_contract.py`，用 `protocol_spec` marker 验证协议文档关键章节、代码映射和变更 checklist。
  - 在 `docs/README.md` 中把协议文档加入 Reference 入口。
- 文件：
  - `docs/reference/protocol.md`
  - `audio-server/tests/acceptance/test_protocol_document_contract.py`
  - `docs/README.md`
- 验证：
...
- 运行证据：
  - `runs/regression-reports/latest/protocol-spec-report.json`
- 风险：
  - 当前协议文档已覆盖主要协议资产，但协议状态机细节仍需要在 Phase 2 中补充成更完整的 L0 测试。

### Phase 1：全层测试标记和命令入口

- 状态：已完成首轮落地
- 目标：给现有核心测试建立 L0/L1/L2/L3 marker，并输出轻量分层测试报告。
- 实现：
  - 在 `pyproject.toml` 中新增 markers：`protocol_spec`、`protocol`、`sdk`、`device_sdk`、`model_provider`、`app`、`replay`、`hardware`。
  - 把 `audio-device/python/tests` 加入 pytest `testpaths`，让 Python Device SDK contract 进入默认可发现范围。
...
- 文件：
  ...
  - `testdata/protocol/invalid/events/target-device-routing.json`
  - `testdata/protocol/invalid/events/unknown-event.json`
  - `conftest.py`
- 验证：
...
- 运行证据：
  ...
  - `runs/regression-reports/latest/summary.md`
- 风险：
  - 当前状态机仍是协议层契约辅助，还没有接入所有运行时服务入口；后续可在 L1 Server SDK 和 Device SDK contract 中复用这些状态机。
...
- 实现：
  - Python Device SDK 事件测试新增对 `testdata/protocol/events` 的全量读取，确认端侧事件对象能消费协议黄金样例。
...
- 验证：
  ...
  - `npm test`（`audio-device/typescript`），结果：`7 pass`。
- 运行证据：
  - `runs/regression-reports/latest/l1-device-sdk-report.json`
  - `runs/regression-reports/latest/l0-protocol-report.json`
  - `runs/regression-reports/latest/summary.md`
- 风险：
  - Swift 当前使用包内 fixture mirror，不是直接引用仓库根目录的 `testdata/protocol`；后续可改成生成/同步机制，避免 fixture 漂移。
  - C 当前原生测试仍主要覆盖本地构造 frame 和注册 JSON，尚未直接读取根目录 `testdata/protocol`。
