...
`audio-chat` 是一套面向多端设备的语音 Agent SDK。它帮助社区开发者快速构建带有语音对话、设备能力调用、跨设备协作和运行排障能力的 AI 应用。
...
第一类是基于大模型的端侧语音交互。应用可以选择文本模型链路，也可以选择 Omni / Realtime 音频链路：
...
> `audio-chat` 是一个用于多端语音 Agent 应用的 Server SDK、Device SDK 和标准通讯协议集合。它把大模型运行时、设备能力调用、stream 传输、Tool / Task 调度、输出播放和运行产物组织成可复用的开发框架。
...
`audio-chat` 不是通用聊天 UI 框架，也不是单纯的语音转文字工具。它不负责：
...
端侧硬件驱动由设备应用自己实现；`audio-chat` 提供的是 server runtime、device 通讯 SDK、标准协议、能力抽象和调试产物。
...
```text
audio-chat
├── audio-server/audio_chat/       # Server SDK，Python 导入名 audio_chat
...
Server SDK 位于 `audio-server/audio_chat/`，负责大模型运行时、设备能力调用和服务端基础设施：
...
这些协议资产位于 `audio-server/audio_chat/spec/` 和 `docs/internal/`。它们主要面向 SDK 维护者、跨语言 SDK 实现者和调试工具。普通应用开发者应该优先使用 Server SDK 和 Device SDK 暴露的能力 API。
...
1. Text Agent Core：使用 ASR、文本模型、Tool loop 和 streaming TTS。
...
SDK 会自动发现并注册这些能力。业务代码不应该写进 `audio-server/audio_chat/` 核心包。
...
5. `legacy/` 只作为迁移参考；新功能应优先使用当前 `audio_chat` 和 `audio-device` API。
