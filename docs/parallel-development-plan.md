...
1. 已有最小 server SDK 包，入口在 `audio-chat-sdk/audio_chat`。
...
4. 从项目根目录运行 `uv run python -m pytest tests -q` 当前通过 64 个测试。
5. 已有设计验收测试、协议验收测试、网络 playback 测试和 provider/output 测试。

不能直接全量并行的原因：

1. 从仓库根目录运行 `uv run python -m pytest tests -q` 当前会因为测试相对路径失败，说明统一验收入口还没有冻结。
...
```text
runs/acceptance/acceptance-result.json
```
...
```text
audio-chat-sdk/audio_chat/protocol.py
