...
当前仓库已经升级为以 `audio-chat` 为主的根目录结构：
...
1. `audio-chat-sdk` 是新 server-side Python SDK 主体。
2. `device-examples` 放参考端侧实现，不属于 SDK 核心源码。
3. `app-examples` 放最小应用和业务样例。
...
1. `audio-chat-sdk` 提供 server SDK 主包、CLI、Agent Core、Tool / Task 扩展面、Control Service、Stream Service、Asset Service、Output Service、回放和预检。
2. `docs` 和 `testdata/contracts` 定义多端必须共同遵守的公共协议、对象模型和契约测试。
3. `device-examples` 只提供参考端侧实现，用来验证协议、降低联调门槛；正式端侧由开发者在自己的工程中自行实现，不要求放入 SDK 目录。
...
如果页面不是从 `127.0.0.1:8765` 同源打开，需要通过 query 参数指定 server 地址，例如 `file:///.../device-examples/browser-glass/index.html?server_url=http://127.0.0.1:8765`。页面点击“连接并注册”和“模拟唤醒”后，应持续上传 16 kHz PCM `sensor.mic` 20ms chunk。server 按 `agent.mode=realtime_audio` 把音频交给 `RealtimeAudioAgentCore`，Qwen Omni Realtime 返回 `response.audio.delta` 后通过 Output Service 原生音频入口下发 `actuator.speaker`，不经过 TextAgentCore ASR 和 TTS。
...
input:
  stream_type: "sensor.mic"
  file: "testdata/provider/dashscope-nihao-16k.pcm"
  codec: "pcm16le"
  sample_rate: 16000
  channels: 1
assertions:
...
uv run python -m audio_chat_python_phone_mock \
  --config device-examples/python-phone/phone.mock.yaml
```
...
uv run audio-chat.esp32.start \
  ...
  --project-dir /path/to/device-examples/native-esp32-glass \
...
```bash
audio-chat.playback.glass \
  --config app-examples/blind-assistant/playback/look_around.yaml
```
...
旧业务和旧 SDK 只作为迁移参考。新能力、新文档和新联调入口优先使用根目录下的 `audio-chat-sdk`、`app-examples` 和 `device-examples`。
