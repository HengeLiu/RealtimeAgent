...
import {
  ...
  DeviceBuilder,
  StreamChunkCodec,
} from "./devices/typescript/src/index.js";
```

### 浏览器页面使用

如果没有打包器，可以把 `devices/typescript/src/*.js` 作为 ESM 模块引入。
...
```bash
cp -R devices/typescript your-project/vendor/realtime-agent-device
```
