...
public struct RealtimeAgentDevice: @unchecked Sendable {
    ...
    public func applying(audioInput: AudioInput, camera: Camera, speaker: Speaker) -> RealtimeAgentDevice {
        ...
        if speaker.enabled {
            ...
            copy.properties["realtime_agent.audio_output.duplex_mode"] = speaker.duplexMode.registrationValue
...
private extension ConversationDuplexMode {
    var registrationValue: String {
        switch self {
        case .playbackOnly:
            return "playback_only"
        case .fullDuplexServerBargeIn:
            return "full_duplex_server_barge_in"
        case .fullDuplexLocalDiagnostic:
            return "full_duplex_local_diagnostic"
        }
    }
}
