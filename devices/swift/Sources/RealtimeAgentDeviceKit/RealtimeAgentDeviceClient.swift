...
/// SDK 对话生命周期状态。
///
/// 主要功能：把 server 仲裁后的 audio session 状态转换成 App 可安全展示的粗粒度状态。
public enum DeviceConversationState: String, Sendable, Equatable {
    case waiting
    case starting
    case active
    case closing
...
public final class RealtimeAgentDeviceClient: @unchecked Sendable {
    ...
    /// 申请当前 SDK 配置需要的硬件权限。
    ///
    /// 主要逻辑：只申请已启用能力对应的权限；麦克风用于 `sensor.mic`，相机用于 `sensor.rgb`。
    /// 返回值：每项权限的授权状态汇总。
    /// 异常情况：系统权限 API 不可用时返回 `.unavailable`，本函数本身不抛出权限拒绝错误。
    public func requestPermissions() async throws -> HardwarePermissionStatus {
        try await RealtimeAgentHardwarePermissionRequester.request(audioInput: audioInput, camera: camera)
    }

    /// 准备硬件和 stream，并请求 server 开始一轮实时对话。
    ///
    /// 主要逻辑：SDK 负责准备麦克风、扬声器和 stream 接收循环，然后发送 `control.user.wake.detected`；
    /// server 仍然负责最终创建 audio session 并下发标准生命周期事件。
    public func startConversation(reason: String = "app_start_requested") async throws {
        if diagnostics.controlState == "disconnected" || diagnostics.controlState == "closed" {
            try await connect()
        }
        ...
        await emitConversationState(.starting)
        try await sendEvent(
    ...
    public func close(force _: Bool = false) async {
    ...
    public func dispatchEvent(_ event: RealtimeAgentEvent) async throws -> Bool {
        ...
        case "stream.output.finish.requested":
            let session = outputSession(for: event)
            await debugLog("speaker finish requested stream=\(session.streamID)")
    ...
    public func dispatchStreamChunk(_ chunk: RealtimeAgentStreamChunk) async throws -> Bool {
        ...
        do {
            ...
            let session = outputSession(for: outputStartEvent(for: chunk))
            try? await failSpeakerOutput(
                session: session,
    ...
    private func handleAudioSessionOpen(_ event: RealtimeAgentEvent) async throws -> Bool {
        ...
        await emitConversationState(.active)
        return true
    }
    ...
    private func outputStartEvent(for chunk: RealtimeAgentStreamChunk) -> RealtimeAgentEvent {
        RealtimeAgentEvent(
    ...
    private func handleOutputChunk(_ chunk: RealtimeAgentStreamChunk) async throws {
        let event = outputStartEvent(for: chunk)
        let session = outputSession(for: event)
    ...
    private func scheduleSpeakerOutputFinish(
        ...
        let task = Task { [weak self, session] in
            ...
            do {
                ...
                try await session.finished(reason: reason)
            } catch is CancellationError {
