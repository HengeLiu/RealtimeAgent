...
actor ConversationStateRecorder {
    private var states: [DeviceConversationState] = []

    func append(_ state: DeviceConversationState) {
        states.append(state)
    }
...
@Test func startConversationRegistersPreparesStreamsAndSendsWake() async throws {
    // 测试目标：确认 App 只调用 SDK 高层 startConversation 时，SDK 会完成注册、准备 stream 并发送 wake。
    // 测试方法：用 mock transport 注入注册成功事件，启用麦克风和扬声器后调用 startConversation。
    // 预期结果：control 事件包含注册请求和 control.user.wake.detected，且音频输入/输出 stream 均被连接。
    let transport = MockRealtimeAgentTransport()
    transport.controlInbox = [
        try eventJSON(
            "control.device.registered",
    ...
    let client = makeClient(
        ...
        speaker: .enabled(sink: RealtimeAgentNoopSpeakerSink())
    )

    try await client.startConversation(reason: "test_start_button")

    let events = try transport.sentControlTexts.map { try RealtimeAgentEvent(jsonString: $0) }
    #expect(events.map(\.eventName) == ["control.device.register.requested", "control.user.wake.detected"])
    #expect(events.last?.payload["reason"] as? String == "test_start_button")
    ...
    #expect(transport.streamConnectedURLs[.audioOutput]?.absoluteString == "ws://127.0.0.1:8765/ws/stream/audio/output?device_id=dev-ios-001")
}
...
@Test func conversationStateCallbackFollowsAudioSessionLifecycle() async throws {
    // 测试目标：确认 App 能通过 SDK 状态回调切换 UI，而不是自行解析标准事件。
    // 测试方法：注册状态 recorder，调用 startConversation 后分发 audio session open/close。
    // 预期结果：状态依次包含 starting、active、closing、waiting。
    let transport = MockRealtimeAgentTransport()
    transport.controlInbox = [
        try eventJSON(
            "control.device.registered",
    ...
    try await client.requestConversationClose(reason: "unit_close")
    #expect(try await client.dispatchEvent(RealtimeAgentEvent(
    ...
    #expect(await recorder.snapshot() == [.starting, .active, .closing, .waiting])
}
...
@Test func standardClientBuildsProfileFromEnabledHardware() async throws {
    ...
    #expect(properties?["realtime_agent.audio_output.duplex_mode"] as? String == "full_duplex_server_barge_in")
...
@Test func outputSessionSendsLifecycleEvents() async throws {
    ...
    let events = try await waitForSentEvent("stream.output.finished", transport: transport)
    let names = events.map(\.eventName)
    #expect(names == ["stream.output.ready", "stream.output.finished"])
}
...
@Test func speakerSinkReceivesOutputChunkThroughSDKBuffer() async throws {
    ...
    // 测试方法：直接分发一帧 24k PCM speaker chunk，不先分发 stream.output.start.requested。
...
@Test func outputFinishWaitsForInFlightSpeakerChunkAndIgnoresTooLateChunks() async throws {
    ...
    _ = try await waitForSentEvent("stream.output.finished", transport: transport)
...
@Test func outputFinishWaitsForExpectedLastSeqAcrossControlAndStreamRace() async throws {
    ...
    // 预期结果：seq=1 仍会进入 speaker sink，且 SDK 最后再发送 stream.output.finished。
    let transport = MockRealtimeAgentTransport()
    let sink = RecordingSpeakerSink()
    let client = makeClient(
        transport: transport,
    ...
    #expect(!namesBeforeLastChunk.contains("stream.output.finished"))
...
@Test func outputCancelPreemptsPendingFinishDrain() async throws {
    ...
    // 预期结果：SDK 先发送 stream.output.cancelled，不发送 stream.output.finished，且 sink 收到 cancel。
    let transport = MockRealtimeAgentTransport()
...
@Test func audioSessionCloseCancelsPendingRgbCapture() async throws {
    // 测试目标：确认 audio session close 会取消尚未完成的 RGB 单帧采集。
    // 测试方法：注册一个会长时间等待的 camera source，收到 RGB 请求后立即分发 audio session close。
    // 预期结果：SDK 只可能发送 input opened/closed 控制回执，不会继续上传 RGB chunk。
    let transport = MockRealtimeAgentTransport()
    let device = RealtimeAgentDevice(deviceID: "dev-ios-001").user("user-001")
    let client = RealtimeAgentDeviceClient(
        serverURL: URL(string: "http://127.0.0.1:8765")!,
    ...
    CameraFrameUploader.registerSingleFrameHandler(client: client, source: source)
    let rgbOpen = RealtimeAgentEvent(
        eventName: "stream.control.open.requested",
        userID: "user-001",
        producerID: "server-main",
        payload: ["stream_type": "sensor.rgb", "request_id": "req-cancel"],
        sessionID: "session-001",
        streamID: "stream-rgb-cancel",
        streamType: "sensor.rgb"
    )
    let audioClose = RealtimeAgentEvent(
        eventName: "control.audio_session.close.requested",
        userID: "user-001",
        producerID: "server-main",
        payload: [:],
    ...
    #expect(try await client.dispatchEvent(rgbOpen))
    try await Task.sleep(nanoseconds: 30_000_000)
    #expect(try await client.dispatchEvent(audioClose))
    try await Task.sleep(nanoseconds: 30_000_000)

    #expect(transport.sentStreamData.isEmpty)
    let names = try transport.sentControlTexts.map { try RealtimeAgentEvent(jsonString: $0).eventName }
    #expect(names.contains("stream.input.opened"))
    #expect(names.contains("control.audio_session.closed"))
}
