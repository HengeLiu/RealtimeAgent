...
def test_yaml_config_matches_design_discovery_and_dev_checks_contract() -> None:
    ...
    测试方法：读取 app-examples/basic-app/server.yaml，检查 tools.discover、tasks.discover 和 dev_checks。
    预期结果：开发者只需把 BaseTool / BaseTask 子类放到配置包下，并运行 dev_checks 完成验收。
    """

    config = load_yaml_config("audio-chat/app-examples/basic-app/server.yaml")
