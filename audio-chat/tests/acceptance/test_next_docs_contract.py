...
ARCHITECTURE_DOC = ROOT / "docs" / "audio-chat-sdk-architecture.md"
...
def test_contract_golden_covers_next_stage_h_categories() -> None:
    """测试目标：确认 H 线路要求的核心契约 golden 已落盘。

    测试方法：检查事件、stream、auth、task lifecycle 和 output arbitration 五类契约。
    预期结果：后续端侧、Task Engine 和 Output Service 修改都有可复用样例。
    """

    required = [
        "testdata/contracts/events/control_device_register_requested.json",
        "testdata/contracts/events/stream_control_configure_requested.json",
        "testdata/contracts/streams/stream_chunk_pcm16le.json",
        "testdata/contracts/auth/signed_token_register_success.json",
        "testdata/contracts/auth/signed_token_register_failed_expired.json",
        "testdata/contracts/tasks/task_lifecycle_rgb_analyze.json",
        "testdata/contracts/output/output_arbitration_preempt_low_priority.json",
    ]
