...
from realtime_agent.audio_pipeline import AudioPipelineConfig, AudioProcessor, AudioProcessorResult, FormatValidator, Pcm16Resampler, VolumeProbe
...
from realtime_agent.realtime_pipeline.base import PipelineEvent
...
class PipelineEventEmitter:
    ...
    def add_listener(self, listener: Callable[[PipelineEvent], None]) -> None:
        ...
        主要逻辑：RealtimeAgentApp 通过监听器消费稳定的 PipelineEvent，把原先散落在
...
class RealtimeOutputController:
    ...
    主要方法：绑定下行、暂停/恢复、取消活跃输出、停止接收新输出和关闭 Vision TTS。
    """
    ...
    def bind_downstream(
        ...
        *,
        ...
        prepare_vision_output: bool = True,
    ) -> None:
        """绑定下行扬声器 stream，并按链路需要预热 Vision TTS session。"""

        self._downstream_by_session[session_id] = stream_id
        self._closed_sessions.discard(session_id)
        if prepare_vision_output:
    ...
    def stop_accepting_new_output(self, *, session_id: str, reason: str) -> None:
        ...
        主要逻辑：现阶段由 VisionRealtimeAgentCore 的 generation 语义阻止旧回复继续输出；这里
