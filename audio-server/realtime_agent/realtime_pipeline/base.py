...
class StreamRef:
    ...
    主要功能：把 `RealtimeAgentApp` 已经完成绑定的 stream 信息传给 pipeline。
...
class RealtimeAgentRealtimePipeline(Protocol):
    """实时音频对话 pipeline 公共接口。

    主要功能：定义 `RealtimeAgentApp` 和 Text / Omni pipeline 之间的稳定调用边界。
