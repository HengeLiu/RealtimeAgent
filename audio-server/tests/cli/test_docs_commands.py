...
def test_docs_entry_points_exist_in_pyproject() -> None:
    ...
    docs = "\n".join(
        [
            ...
            (AUDIO_ROOT / "audio-server" / "docs" / "how-to" / "device-capability-development.md").read_text(encoding="utf-8"),
        ]
    )
    required = {
...
def test_for_blind_server_yaml_documents_supported_model_routes(tmp_path) -> None:
    ...
    config = AUDIO_ROOT / "examples" / "for-blind-app" / "audio-server" / "server.yaml"
...
def test_developer_context_device_design_doc_covers_target_contracts() -> None:
    ...
    guide = (AUDIO_ROOT / "audio-server" / "docs" / "reference" / "context-api.md").read_text(encoding="utf-8")
    required = [
        "设计说明",
        "不是当前版本的开发操作手册",
        "ToolContext",
...
def test_device_capability_development_guide_covers_current_workflow() -> None:
    ...
    guide = (AUDIO_ROOT / "audio-server" / "docs" / "how-to" / "device-capability-development.md").read_text(encoding="utf-8")
    required = [
        "当前仓库已经可用",
        "supports",
        "structured supports",
