...
本目录覆盖 Server SDK 的 Agent Core 与模型适配边界。测试可使用 fake provider，但不 mock SDK 内部核心流程。

| 文件 | 测试目标和范围 |
| --- | --- |
| `test_agent_core_recovery.py` | 验证 Agent Core 错误恢复和降级输出。 |
| `test_agent_core_router.py` | 验证文本 / realtime Agent Core 路由选择和配置传递。 |
