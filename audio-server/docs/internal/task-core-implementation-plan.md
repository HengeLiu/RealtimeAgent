...
1. `uv run python -m pytest audio-server/tests/test_task_engine_scheduler.py audio-server/tests/test_task_engine_persistence.py audio-server/tests/test_task_manage_tool.py audio-server/tests/acceptance/test_architecture_design_contract_acceptance.py examples/for-blind-app/tests/test_phone_task_contract.py examples/for-blind-app/tests/test_peer_video_tasks.py -q`
   - 结果：26 passed。

补充说明：

1. 本轮也尝试运行 `uv run python -m pytest audio-server/tests -q`，但当前工作树存在多类与 Task Core 无直接关系的既有失败，包括文档状态矩阵、CLI 帮助输出、ControlService 路由、输出播放、ASR 输入流等。上述失败未在本轮处理。
