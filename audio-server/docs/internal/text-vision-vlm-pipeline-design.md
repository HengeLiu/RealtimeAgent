...
```text
sensor.mic -> ASR -> TextAgentCore -> Text Model -> Streaming TTS -> actuator.speaker
```

接下来 Text 类目中的图片理解、视频理解不应该再设计成“Text 模型调用另一个视觉模型 Tool 辅助理解”。用户指定的 `qwen3.6-flash` 本身就是多模态大模型，可以在同一次模型请求中接收文本、图片和视频内容。因此新的主方案是：

```text
sensor.mic -> ASR -> TextAgentCore -> MultimodalMessageBuilder
  -> qwen3.6-flash(messages with image/video content blocks)
  -> Streaming TTS
```

也就是说，图像 / 视频理解属于 Text 主模型请求的一部分。系统只需要在模型明确请求视觉资产后采集或选择资产，并把资产以 content block 的形式拼到后续 message 中；不需要额外调用 `interpret_image`、`interpret_video` 这类“理解工具”。

## 核心结论

1. `qwen3.6-flash` 是 Text 类目的主模型，也是图片 / 视频理解模型。
2. 图片 / 视频不再通过“视觉理解 Tool”单独调用模型；理解发生在 TextAgentCore 发给主模型的同一次请求里。
3. Tool 只负责获取资产，例如拍照、选择视频、截取当前帧；Tool 不负责理解图片内容。
4. `TextAgentCore` 复用现有 `ContextCompiler / ModelContext` 做基础消息管理，并增加同轮工具结果后的多模态 follow-up message 拼接。
5. 模型返回的文本 delta 继续走现有 Text realtime TTS 链路。
...
- Text 链路已经通过 `ContextCompiler.compile(mode="text")` 获得当前轮 `messages`，再交给 Text provider 流式调用。
- Realtime Omni 链路也使用 `ContextCompiler.compile(mode="realtime_audio")` 生成等价请求视图和运行产物。

因此 Text 多模态不应该重新实现一套完整历史消息系统，而应该复用这层共享能力。

不能直接复用的是 Omni provider 内部的 conversation item 管理。Omni 的 `_conversation.create_item()`、`append_audio()`、`append_video()`、`commit()`、`response.create()` 处理的是 Realtime provider 的会话事件和音频 / 视频提交语义；Text provider 使用的是 Chat Completions 风格的 `messages` 数组和 tool call follow-up 语义，两者生命周期不同。

新的 Text 视觉链路应新增一层 `ModelMessageManager` 或 `MultimodalMessageBuilder`，职责限定为：

1. 基于 `ContextCompiler` 输出的 `ModelContext.messages` 生成 Text provider 请求。
...
`capture_photo` 保留并默认暴露给 Text 主模型，但语义收窄：
...
以下能力不再作为 Text 多模态主路径：

- `interpret_current_view`
- `interpret_image`
- `interpret_video`

它们可以保留为兼容或调试工具，但默认不应该暴露给 Text 主模型；否则模型会先调用工具，再由工具调用另一个模型，造成链路重复、延迟增加、上下文不一致。
...
```text
ASR final
  -> TextAgentCore 编译纯文本请求，工具列表包含 capture_photo
...
首版应把 `capture_photo` 从 Text denylist 中移除，同时继续默认隐藏 `interpret_current_view`、`interpret_image`、`interpret_video`。
...
如果后续 Text 链路也接入类似 Omni 的 visual sampler，且当前语音 turn 内已经采集过新帧，则可直接使用该帧，不重复请求摄像头。
...
TextAgentCore 保留标准 tool result message：
...
建议新增 Text 多模态 message 管理模块，位置在 SDK core 的 Agent Core 边界内，而不是 for-blind Tool 内：

```text
audio-server/audio_chat/agent_core/multimodal/
  __init__.py
  assets.py
  builder.py
  messages.py
...
| `messages.py` | 管理 Text provider 的 tool call、tool result 和多模态 follow-up messages。 |
| `video_sampling.py` | 视频抽帧、压缩和时间戳管理。 |
| `policy.py` | 限制最大图片数、视频大小、freshness、单轮抓拍次数和资产类型。 |

TextAgentCore 只调用一个高层接口：
...
建议在 `examples/for-blind-app/audio-server/server.yaml` 中把 Text 主模型升级为多模态模型：
...
1. `agent.text.model` 改为 `qwen3.6-flash`。
2. 增加 `agent.text.multimodal` 配置。
...
- 不把连续 peer video 全量塞进 Text 模型请求。
- 不用单轮 VLM 替代找物、红绿灯和持续观察 Task。
- 不在控制事件里传图片 / 视频 bytes。

## 关键决策

1. Text 类目图片 / 视频理解由 `qwen3.6-flash` 主模型完成。
...
- 状态：已完成。
- 实现：
  - `AgentTextConfig` 增加 `multimodal` 配置，运行时同步到 `AudioChatConfig`。
  - 示例应用 Text 主模型切到 `qwen3.6-flash`。
...
- 文件：
  ...
  - `audio-server/audio_chat/preflight.py`
  - `examples/for-blind-app/audio-server/server.yaml`
- 验证：
  - `uv run python -m pytest audio-server/tests/test_config_sync.py -q` 已通过。
  - `uv run audio-chat.dev.preflight --config examples/for-blind-app/audio-server/server.yaml --report runs/default-app/text-vlm-preflight.json` 已通过。

### Phase 2：Message 管理和工具结果图片拼接

- 状态：已完成。
- 实现：
  - 新增 `audio_chat.agent_core.multimodal` 模块，包含 policy、assets、builder、messages 和 video_sampling。
  - `TextAgentCore` 初始化 `ModelMessageManager`，复用 `ContextCompiler` 输出的基础 messages。
...
- 文件：
  ...
  - `audio-server/audio_chat/agent_core/router.py`
- 验证：
  - `uv run python -m pytest audio-server/tests/test_text_agent_tool_loop_async.py -q` 已通过。
