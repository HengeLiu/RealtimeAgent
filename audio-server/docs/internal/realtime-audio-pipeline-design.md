...
本文定义服务器内部 `Realtime Audio Pipeline` 的完整设计，目标是让 Text Realtime 和 Omni Realtime 两条链路对外表现为同一种实时音频对话能力。

端侧不应该知道服务器内部使用的是 Text 模型链路还是 Omni 模型链路。对 `AudioChatApp`、`ControlService`、`StreamService` 和端侧设备来说，两条链路都必须满足同一组外部行为：
...
Text 链路原始标准见：[文本模型实时音频链路处理标准.md](文本模型实时音频链路处理标准.md)。

## 设计原则

### 1. pipeline 对外统一

服务器外部调用方只能依赖 `AudioChatRealtimePipeline` 这一层抽象，不直接依赖 `TextAgentCore`、`OmniAgentCore`、Paraformer、TTS provider 或 Omni provider 的原始事件。

Text 和 Omni 的差异必须封装在各自 pipeline 内部：

| 链路 | pipeline 实现 | Agent Core | 输入边界来源 | 回复来源 |
| --- | --- | --- | --- | --- |
| Text Realtime | `TextRealtimePipeline` | `TextAgentCore` | `paraformer-realtime-v2` 句子事件 | Text LLM + Streaming TTS |
...
```text
mic audio -> Paraformer ASR -> TextAgentCore -> Text LLM -> Streaming TTS -> speaker audio
```
...
`AudioChatApp` 调用 `attach_upstream` 或 `attach_downstream` 之前，必须已经完成以下绑定：
...
用户插话不是异常。无论 Text 还是 Omni，只要 pipeline 判断用户开始说话，且当前存在正在生成或播放的助手输出，就必须执行统一打断语义：
...
`AudioChatRealtimePipeline` 对 `AudioChatApp` 暴露以下接口：

| 方法 | 触发方 | 语义 |
| --- | --- | --- |
| `open_session(user_id, session_id)` | `AudioChatApp` | 打开一次连续对话 session，初始化内部状态。 |
| `attach_upstream(stream_ref)` | `AudioChatApp` | 绑定上行麦克风长连接，并预热输入 provider。 |
| `attach_downstream(stream_ref)` | `AudioChatApp` | 绑定下行扬声器长连接，并预热输出 provider。 |
| `append_input_audio(chunk)` | `AudioChatApp` | 持续写入上行麦克风音频。 |
| `pause_downstream()` | `AudioChatApp` | 端侧下行队列达到高水位，暂停继续发送下行音频。 |
| `resume_downstream()` | `AudioChatApp` | 端侧下行队列回落到低水位，恢复发送下行音频。 |
| `notify_output_finished(stream_id)` | `AudioChatApp` | 端侧确认本段 output stream 已完成播放 drain。 |
| `detach_upstream(reason)` | `AudioChatApp` | 上行连接关闭，释放输入 provider。 |
| `prepare_close(reason)` | `AudioChatApp` | 准备关闭连续对话，停止接收新输出并 drain 或取消当前输出。 |
| `close_session(reason)` | `AudioChatApp` | 关闭 session，释放 provider 和内部状态。 |

### 统一事件

pipeline 对 `AudioChatApp` 只输出统一 `PipelineEvent`：

| 事件 | 语义 | 外部处理 |
| --- | --- | --- |
| `session_ready` | pipeline session 初始化完成。 | `AudioChatApp` 可等待上下行连接。 |
...
| `session_closed` | pipeline 内部状态和 provider 已释放。 | `AudioChatApp` 完成 session 清理。 |

### 外部契约时序

```plantuml
!include audio-chat-realtime-pipeline-contract.puml
```
...
| 组件 | 职责 |
| --- | --- |
| `AudioChatApp` | 应用编排入口，负责把控制事件、stream 事件和 pipeline 事件串起来。 |
| `ControlService` | 负责控制事件发布和端侧路由。 |
| `StreamService` | 负责 stream 生命周期、chunk 写入、finish、cancel 和端侧回执。 |
| `AudioChatRealtimePipeline` | Text / Omni 的统一接口，不暴露 provider 细节。 |
...
| `PipelineEventEmitter` | 统一输出 `PipelineEvent`，避免 Text/Omni 原始事件穿透到外部。 |
...
## TextRealtimePipeline 设计

### 适用场景

`TextRealtimePipeline` 用于文本模型驱动的实时语音对话。它把实时麦克风音频转换成文本 turn，再由文本模型生成回复，最后通过流式 TTS 生成下行音频。

### 内部组件

| 组件 | 职责 |
| --- | --- |
| `TextRealtimePipeline` | 实现 `AudioChatRealtimePipeline` 统一接口，协调 Text 专属组件和共享组件。 |
| `TextAgentCore` | Text 链路对话语义核心，负责上下文、消息历史、turn 状态、打断、关闭请求。 |
| `TextInputBoundary` | 解释 Paraformer 事件，把句子事件转换成统一输入边界事件。 |
| `TextResponseEngine` | 管理 Text LLM 流式请求和 Streaming TTS。 |
| `Paraformer Realtime V2` | Text 链路的 ASR 和用户语音边界来源。 |
| `Text Model Provider` | 生成文本回复和 tool call。 |
| `Streaming TTS Provider` | 把文本 delta 合成下行音频 delta。 |

### 会话初始化

`open_session` 时，`TextAgentCore` 需要完成：

1. 编译当前会话上下文。
2. 初始化消息 buffer。
3. 初始化 generation id。
4. 设置状态为 `listening`。
5. 准备 `TextInputBoundary` 和 `TextResponseEngine`。
...
Text 链路不使用端侧 VAD。用户是否开始说话、停止说话由 Paraformer 返回的句子事件解释得到：

| Paraformer 事件 | TextInputBoundary 输出 |
| --- | --- |
...
当 `TextAgentCore` 收到最终用户文本后：

1. 写入 user message。
2. 启动 `TextResponseEngine.start_response()`。
3. `TextResponseEngine` 请求文本模型。
4. 每个 text delta 追加到 assistant buffer。
5. text delta 推入 TTS。
6. TTS audio chunk 进入 `RealtimeOutputController`。
7. pipeline 发出 `output_audio_delta`。
8. 文本模型和 TTS 完成后，pipeline 发出 `output_finished`。

### 打断处理

Text 链路中，打断由新的 `sentence_begin=true` 触发。
...
1. listening 状态空闲超时。
2. 文本模型调用 `close_audio_session` Tool。

模型调用关闭 Tool 时，`TextAgentCore` 必须写入 assistant tool call message 和 tool result message，再发出 `PipelineEvent.close_requested`。
...
```plantuml
!include text-realtime-server-side-sequence.puml
```
...
| 组件 | 职责 |
| --- | --- |
| `OmniRealtimePipeline` | 实现 `AudioChatRealtimePipeline` 统一接口，协调 Omni 专属组件和共享组件。 |
...
provider 原始事件不能穿透到 `AudioChatApp`。`AudioChatApp` 只能看到统一的 `PipelineEvent.speech_started` 和 `PipelineEvent.speech_stopped`。
...
| 配置语义 | 作用 |
| --- | --- |
| ASR 句子等待时间 | Text 链路传给 `paraformer-realtime-v2`，用于判断一句话结束。 |
...
`Realtime Audio Pipeline` 的目标是把 Text / Omni 的实时音频对话语义收敛到同一层，而不是把 WebSocket、控制事件、stream 生命周期全部搬进 pipeline 内部。

### 不应该做的事情

1. 不在端侧实现连续对话阶段 VAD。
2. 不在 pipeline 热路径重复做连接身份校验。
3. 不让 Omni provider 原始事件穿透到 `AudioChatApp`。
4. 不让 Text 链路的 Paraformer 原始事件穿透到 `AudioChatApp`。
...
1. Text 和 Omni 都通过同一组 `AudioChatRealtimePipeline` 接口接入 `AudioChatApp`。
2. 端侧只需要实现 [端侧音频交互标准.md](端侧音频交互标准.md)，不需要关心 Text / Omni 差异。
3. Text 链路上行连接建立时预热 ASR，下行连接建立时预热 TTS。
...
8. 连续对话关闭时，Text 释放 ASR 和 TTS，Omni 释放 Omni provider session。
9. 空闲超时和模型关闭请求都能触发 `close_requested`。
10. 下行水位控制对 Text 和 Omni 使用同一套 pause / resume 接口。
