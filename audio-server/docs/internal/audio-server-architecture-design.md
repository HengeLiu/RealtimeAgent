...
5. `legacy` 保存旧 `openaiglass-sdk`、旧 `openaiglass-for-blind` 和历史资料，只作为迁移参考。
...
| 模块                   | 状态     | 当前依据                                                                                                                                                                                                                                                                                                                       |
| ---------------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Control Service        | 已实现   | 设备注册、订阅匹配、事件发布和 debug API 已由 `audio-server/protocol-tests/sdk/runtime/test_control_service.py`、`tests/acceptance/test_protocol_routing_acceptance.py` 覆盖。                                                                                                                                                                               |
| Stream Service         | 已实现   | stream chunk 编码、输入/输出 stream 生命周期和网络 playback 已由 `audio-server/protocol-tests/sdk/runtime/test_stream_and_audio_pipeline.py`、`examples/dev-support/app-tests/network/test_network_server_playback.py` 覆盖。                                                                                                                                                                 |
...
| ToolGateway            | 已实现   | `BaseTool`、自动发现、策略、schema、执行和 trace 记录已由 `audio-server/protocol-tests/acceptance/test_auto_discovery_developer_contract.py`、`audio-server/protocol-tests/sdk/runtime/test_tool_spec_schema.py` 与 `audio-server/protocol-tests/sdk/agent_core/test_agent_core_router.py` 覆盖。                                                                                                                                                      |
| Task Engine            | 部分实现 | 已有 `BaseTask`、状态机、TaskEventBridge 和最小执行器；持久化、恢复、超时、并发限制属于 D 线路。                                                                                                                                                                                                                             |
| Output Service         | 已实现   | 文本输出、原生音频输出、播放仲裁、通知协调和 output stream 已由 `audio-server/protocol-tests/sdk/runtime/test_phase2_providers_output.py` 覆盖。                                                                                                                                                                                                               |
...
| Memory / Skill / MCP   | 已实现   | C 线路已落地 `MemoryService`、`SkillService`、`McpGateway` 和内置 Tool；由 `audio-server/protocol-tests/sdk/runtime/test_memory_service.py`、`audio-server/protocol-tests/sdk/extensions/test_skill_service.py`、`audio-server/protocol-tests/sdk/extensions/test_mcp_gateway.py`、`audio-server/protocol-tests/acceptance/test_indirect_device_context_contract.py` 覆盖。任何需要设备通讯能力的 Memory / Skill / MCP 都必须封装成 Tool 或 Task。 |
...
| 能力                        | 验收依据                                                                                                                                                                                                                       |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| CLI 和开发者入口            | `audio-server/unit-tests/cli/test_cli_developer_workflow.py`、`audio-server/unit-tests/cli/test_docs_commands.py`                                                                                                                                                      |
| 自动发现 Tool / Task        | `audio-server/protocol-tests/acceptance/test_auto_discovery_developer_contract.py`                                                                                                                                                                 |
| 设备注册与订阅分发          | `audio-server/protocol-tests/sdk/runtime/test_control_service.py`                                                                                                                                       |
| stream chunk 协议           | `protocol/protocol-tests/test_protocol_contracts.py`、`audio-server/protocol-tests/sdk/runtime/test_stream_and_audio_pipeline.py`                                                                                 |
| 资产缓存与能力回放          | `examples/for-blind-app/app-tests/acceptance/test_capability_template_playback.py`                                                                                                                  |
| Tool / Task 协议原生扩展    | `audio-server/protocol-tests/acceptance/test_auto_discovery_developer_contract.py`、`audio-server/protocol-tests/sdk/runtime/test_tool_spec_schema.py`、`audio-server/protocol-tests/acceptance/test_task_device_stream_contract.py`                                                                                                                                                                |
| Vision 模型音频闭环           | `examples/for-blind-app/replay-tests/test_vision_route_audio_samples.py`                                                                                                                                                                                     |
| Output Service 与播放仲裁   | `audio-server/protocol-tests/sdk/runtime/test_phase2_providers_output.py`                                                                                                          |
| H 线路迁移样板              | `examples/for-blind-app/audio-server/capabilities/tools.py`、`examples/for-blind-app/audio-server/capabilities/tasks.py`、`examples/for-blind-app/app-tests/acceptance/test_migration_template_contract.py`                                                                                                                             |
| Memory / Skill / MCP 能力面 | `audio-server/protocol-tests/sdk/runtime/test_memory_service.py`、`audio-server/protocol-tests/sdk/extensions/test_skill_service.py`、`audio-server/protocol-tests/sdk/extensions/test_mcp_gateway.py`、`audio-server/protocol-tests/acceptance/test_indirect_device_context_contract.py`                                                                |
...
| `task_runtime_manager`   | 查询、取消和列出 Task 运行实例。                                               |
...
Task 不直接继承 `BaseTool`。Task 注册到 `TaskEngine` 后，SDK 根据 `TaskSpec` 自动生成一个模型可见启动 Tool，例如 `timer_task` 生成 `start_timer_task`。启动 Tool 的 provider schema 来自 `TaskSpec.input_model`，且与普通 Tool 使用同一套 schema 规则：推荐并默认样板使用 Pydantic `BaseModel`，字段类型、必填项、默认值、取值范围和 `Field(description=...)` 会传给模型；JSON Schema dict 仅作为兼容入口。模型调用启动 Tool 后，SDK 内部再执行 `TaskEngine.create()`。`task_runtime_manager` 保留为运行时管理 Tool，只负责 `list_types/query/cancel/list_instances`。
...
| 对象             | 说明                                                                                  |
| ---------------- | ------------------------------------------------------------------------------------- |
| `TaskSpec`     | 某类任务模板，包含 `task_type`、`input_model`、`start_tool_name`、`version`、取消能力和超时设置。 |
...
1. 默认名称为 `start_{task_type}`；如果 `task_type` 不以 `_task` 结尾，则使用 `start_{task_type}_task`。
2. 开发者可通过 `BaseTask.start_tool_name` 覆盖默认名称。
3. 启动 Tool 的参数 schema 来自 `BaseTask.input_model`，推荐使用 Pydantic `BaseModel`；JSON Schema dict 仅用于兼容旧实现或外部生成 schema。
4. 启动 Tool 返回 `TaskRef`，并把 `task_id` 写入 `ToolResult.tasks`，便于后续 `task_runtime_manager` 查询或取消。
5. 启动 Tool 是模型可见工具；`TaskEngine`、`TaskContext` 和调度器仍然不直接暴露给模型。
...
input:
  stream_type: "sensor.mic"
  file: "audio-server/model-provider-tests/fixtures/provider/dashscope-nihao-16k.pcm"
  codec: "pcm16le"
  sample_rate: 16000
  channels: 1
assertions:
...
旧业务和旧 SDK 只作为迁移参考。新能力、新文档和新联调入口优先使用根目录下的 `audio-server`、`examples` 和 `docs`。
