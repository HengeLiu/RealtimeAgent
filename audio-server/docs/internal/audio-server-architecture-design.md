...
| 模块                   | 状态     | 当前依据                                                                                                                                                                                                                                                                                                                       |
| ---------------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Control Service        | 已实现   | 设备注册、订阅匹配、事件发布和 debug API 已由 `audio-server/tests/test_control_service.py`、`tests/acceptance/test_protocol_routing_acceptance.py` 覆盖。                                                                                                                                                                               |
| Stream Service         | 已实现   | stream chunk 编码、输入/输出 stream 生命周期和网络 playback 已由 `audio-server/tests/test_stream_and_audio_pipeline.py`、`examples/dev-support/tests/test_network_server_playback.py` 覆盖。                                                                                                                                                                 |
| Audio Pipeline         | 部分实现 | 已有格式校验、mock provider 和最小音频链路；text 路线可复用 AudioSample 做无头回放，见 `examples/for-blind-app/tests/test_text_route_audio_samples.py`；完整 wake 后会话生命周期、打断和清理属于 A 线路。                                                                                                                                           |
...
| 能力                        | 验收依据                                                                                                                                                                                                                       |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| CLI 和开发者入口            | `audio-server/tests/test_cli_developer_workflow.py`、`audio-server/tests/test_docs_commands.py`                                                                                                                                                      |
| 自动发现 Tool / Task        | `audio-server/tests/acceptance/test_auto_discovery_developer_contract.py`                                                                                                                                                                 |
| 设备注册与订阅分发          | `audio-server/tests/test_control_service.py`                                                                                                                                       |
| stream chunk 协议           | `audio-server/tests/test_protocol_contracts.py`、`audio-server/tests/test_stream_and_audio_pipeline.py`                                                                                 |
...
input:
  stream_type: "sensor.mic"
  file: "audio-server/tests/fixtures/provider/dashscope-nihao-16k.pcm"
  codec: "pcm16le"
  sample_rate: 16000
  channels: 1
assertions:
