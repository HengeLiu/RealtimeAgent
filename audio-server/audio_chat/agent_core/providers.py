...
class MockTextModelAdapter:
    ...
    def __init__(self, model: str = "mock-text", *, prompt: str = TEXT_AGENT_SYSTEM_PROMPT) -> None:
        self.model = model
        self.prompt = prompt
...
def build_text_model(config: TextModelProviderConfig) -> tuple[TextModelAdapter, str | None]:
    try:
        if config.provider == "mock":
            return MockTextModelAdapter(model=config.model, prompt=config.prompt), None
