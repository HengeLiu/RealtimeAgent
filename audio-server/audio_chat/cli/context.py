...
def inspect() -> None:
    ...
    参数：通过命令行传入 config、mode、user_id、session_id 和可选文本输入。
    返回值：无；结果输出到 stdout。
    异常情况：配置或 prompt registry 错误时抛出并由命令行显示。
    """

    parser = argparse.ArgumentParser(prog="audio-chat.context.inspect")
    parser.add_argument("--config", required=True, help="server.yaml 路径")
    parser.add_argument("--mode", choices=["text", "realtime_audio"], default="", help="要检查的 Agent 模式")
    ...
    parser.add_argument("--text", default="你好", help="text 模式当前用户输入")
    parser.add_argument("--compare-model-request", default="", help="可选：与已有 model-request.json 做摘要 diff")
    args = parser.parse_args()

    config = AudioChatConfig.from_yaml(Path(args.config))
    app = AudioChatApp(config)
    mode = args.mode or ("realtime_audio" if config.agent_mode == "realtime_audio" else "text")
    compiler = ContextCompiler()
    if mode == "text":
        provider = config.text_provider
        model = config.text_model
        base_instructions = config.text_prompt
        ...
        base_instructions = config.realtime_prompt
    ...
    context = compiler.compile(
        ContextCompileRequest(
            ...
            max_context_messages=config.text_max_context_messages,
        )
    )
    payload = {
        "mode": context.mode,
