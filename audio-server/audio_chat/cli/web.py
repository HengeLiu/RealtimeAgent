...
def open_web(argv: list[str] | None = None) -> None:
    ...
    parser = argparse.ArgumentParser(prog="audio-chat.web.open", description="打开 audio-chat browser-glass 设备示例")
    ...
    parser.add_argument("--server-url", default="http://127.0.0.1:8765", help="browser-glass 要连接的 audio-chat server URL")
...
def _serve_web(*, html_path: Path, host: str, port: int, print_url: bool, server_url: str) -> None:
    ...
    `audio-device/typescript`。`--print-url` 模式只打印 HTTP URL 后立即退出，便于测试。
    参数：`html_path` 为页面路径，`host/port` 为监听地址，`print_url` 控制是否只打印，
    `server_url` 为页面连接的 audio-chat 服务地址。
    返回值：无。
