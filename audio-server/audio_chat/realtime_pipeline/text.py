...
class TextInputBoundary:
    ...
    主要属性：`core` 是实际持有 ASR pipeline 和语音边界解释逻辑的 `TextAgentCore`。
    """

    def __init__(self, *, core: TextAgentCore, emitter: PipelineEventEmitter) -> None:
        self.core = core
    ...
    def _install_core_boundary_bridge(self) -> None:
        """把 TextAgentCore 的语音边界事件转换为统一 PipelineEvent。

        主要逻辑：Paraformer 事件仍由 `TextAgentCore` 解释，避免复制 ASR 细节；
        `TextInputBoundary` 只负责把解释后的 speech start/stop 桥接成 pipeline
...
class TextResponseEngine:
    """Text 链路回复生成组件。

    主要功能：封装 Text 模型和 Streaming TTS 回复生成能力。当前实现复用
    `TextAgentCore` 内部成熟的工具循环、上下文编译和文本输出逻辑。
    主要方法：`prepare_session()` 记录回复引擎准备；`cancel_current_response()`
    取消当前 generation；`close()` 释放模型和 TTS 相关状态。
    """

    def __init__(self, *, core: TextAgentCore, output_controller: RealtimeOutputController, emitter: PipelineEventEmitter) -> None:
        self.core = core
...
class TextRealtimePipeline:
    """Text realtime 音频对话 pipeline。

    主要功能：实现 `AudioChatRealtimePipeline` 设计中的 Text 链路，把现有
    `TextAgentCore`、ASR、文本模型、TTS 和 OutputService 组合成真实 pipeline 组件。
    ...
    def __init__(
        ...
        *,
        ...
        object.__setattr__(self, "input_boundary", TextInputBoundary(core=core, emitter=emitter))
        object.__setattr__(
            self,
            "response_engine",
            TextResponseEngine(core=core, output_controller=output_controller, emitter=emitter),
        )
        object.__setattr__(self, "_session_by_user", {})
        object.__setattr__(self, "_downstream_by_session", {})

    def __getattr__(self, name: str) -> Any:
        """把旧调用点透明代理到内部 TextAgentCore。

        主要逻辑：现有测试和少量应用代码会直接访问 `text_model/asr_pipeline` 等属性；
    ...
    def __setattr__(self, name: str, value: Any) -> None:
        """把旧 TextAgentCore 属性写入代理到内部 core。"""
