...
class OmniAgentCore(RealtimeAudioAgentCore):
    """Omni Realtime 链路的 Agent Core。

    主要功能：作为设计文档中的 `OmniAgentCore` 真实代码组件，复用现有
    `RealtimeAudioAgentCore` 的 provider 连接、工具调用、视觉帧追加和消息记录能力。
...
class OmniRealtimePipeline:
    ...
    主要功能：实现 `AudioChatRealtimePipeline` 设计中的 Omni 链路，把 Omni provider
