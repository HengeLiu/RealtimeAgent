# Vision Realtime 服务器侧标准模型

## 目标

本文把服务器中 Vision Realtime 语音链路定为标准实现模型。
...
本文适用于 Vision Realtime 链路：

```text
sensor.mic -> AudioPipeline -> Paraformer Realtime V2 -> VisionRealtimeAgentCore -> VisionModelProvider -> Streaming TTS -> actuator.speaker
```

不适用于 Omni Realtime 链路。Omni Realtime 的 speech start / speech stop 由 Omni provider 直接给出；Vision Realtime 的 speech start / speech stop 由 Paraformer realtime ASR 结果解释得到。
...
| 组件 | 代码职责 |
| --- | --- |
| `RealtimeAgentServer.control_ws` | 处理端侧控制 WebSocket，例如设备注册、唤醒、audio session open / close、端侧回执。 |
| `RealtimeAgentServer.stream_ws` | 处理端侧音频数据 WebSocket，包括上行麦克风连接和下行扬声器连接。 |
| `RealtimeAgentApp` | 服务器应用编排入口，把 WebSocket 事件转交给控制、数据流、Agent Core 和输出服务。 |
...
| `VisionRealtimeAgentCore` | Vision Realtime 状态机，负责听说状态、ASR 事件解释、LLM 请求、上下文写入、打断和关闭连续对话。 |
...
| `VisionModelProvider` | 文本大模型 provider，负责流式输出 `vision_delta` 和 tool call。 |
...
如果连接不能被绑定到合法设备和合法 session，应在连接建立阶段失败，而不是让音频帧进入 Vision Realtime 后再补充校验。

### 2. Provider 连接必须提前建立

为了降低首轮响应延迟，provider 连接不能等到首帧音频或首段文本到达时才建立。

上行麦克风连接建立后，`VisionRealtimeAgentCore` 必须让 `AsrPipeline` 预先创建 ASR provider：

```text
VisionRealtimeAgentCore -> AsrPipeline -> DashScopeAsrProviderAdapter -> paraformer-realtime-v2
```
...
```text
RealtimeAgentServer.stream_ws -> RealtimeAgentApp -> AudioPipeline -> VisionRealtimeAgentCore -> AsrPipeline -> DashScopeAsrProviderAdapter
```
...
用户开始说话、停止说话、是否打断当前回复，由 `VisionRealtimeAgentCore` 根据 Paraformer provider 事件解释后决定。

### 4. Paraformer Realtime V2 是 Vision 链路的 ASR 和语音边界来源

Vision Realtime 使用 `paraformer-realtime-v2` 作为 ASR provider。

该 provider 不返回独立的 `speech_started` / `speech_stopped` 事件。服务器必须使用 `result-generated.output.sentence` 中的字段解释语音边界：

| Paraformer 字段 | 服务器语义 |
| --- | --- |
| `sentence_begin=true` | 用户开始说话。`VisionRealtimeAgentCore` 发布 `audio.speech.started`。 |
| partial `text` | ASR 中间文本。只作为 transcript delta，不单独承担 VAD 职责。 |
| `sentence_end=true` 或带 `end_time` 的最终句子 | 用户停止说话，并形成一轮最终输入。`VisionRealtimeAgentCore` 发布 `audio.speech.stopped`，然后启动视觉语言模型响应。 |
...
1. 端侧通过 `RealtimeAgentServer.control_ws` 注册设备。
...
5. `RealtimeAgentApp` 打开 `VisionRealtimeAgentCore` session。
...
2. `AsrPipeline` 把 provider 事件交给 `VisionRealtimeAgentCore`。
3. `VisionRealtimeAgentCore` 发布 `audio.speech.started` 给端侧。
4. 如果当前存在正在生成或播放的助手输出，`VisionRealtimeAgentCore` 通知 `OutputService` 打断。
...
2. `VisionRealtimeAgentCore` 发布 `audio.speech.stopped`。
3. `VisionRealtimeAgentCore` 写入用户消息。
4. `VisionRealtimeAgentCore` 启动新一轮视觉语言模型响应。

### 3. 视觉语言模型和流式 TTS

`VisionRealtimeAgentCore` 收到最终用户输入后，向 `VisionModelProvider` 发起流式请求。
...
1. `VisionRealtimeAgentCore` flush 当前输出。
2. `VisionRealtimeAgentCore` 把完整 assistant message 写入消息历史。
3. `OutputService` 对当前 output stream 发出 `finish` 语义。
4. 端侧播放队列 drain 完成后回报 `stream.output.finished`。
5. `VisionRealtimeAgentCore` 回到 listening 状态。
...
1. `VisionRealtimeAgentCore` 发布 `audio.speech.started`。
2. `VisionRealtimeAgentCore` 把当前 generation 已经生成的文本追加到 assistant message。
3. 该 assistant message 的内容末尾必须追加 `<用户打断>`。
4. `VisionRealtimeAgentCore` 通知 `OutputService` cancel 当前 speaker stream。
5. `OutputService` 停止继续向端侧发送旧回复音频。
6. 旧 generation 后续返回的 LLM / TTS 回调只能丢弃，不能写成 `response.failed`，也不能播放错误兜底音频。
7. 新用户句子结束后，`VisionRealtimeAgentCore` 启动下一轮模型响应。
...
当 `VisionRealtimeAgentCore` 处于 listening 状态，且在配置的超时时间内没有收到新的有效用户语音输入时，服务器主动关闭连续对话。
...
1. 用户最终文本进入 `VisionRealtimeAgentCore`。
2. `VisionRealtimeAgentCore` 请求视觉语言模型。
3. 视觉语言模型根据上下文调用关闭连接 Tool。
4. `VisionRealtimeAgentCore` 写入 tool call 和 tool result 消息。
5. `VisionRealtimeAgentCore` 发布 `control.audio_session.close.requested`。
...
1. `VisionRealtimeAgentCore` 发布 `control.audio_session.close.requested`。
...
10. `RealtimeAgentApp` 关闭当前 session，`VisionRealtimeAgentCore` 清理状态。
...
```plantuml
@startuml
title Vision Realtime 服务器侧时序（现有代码组件 + Paraformer Realtime V2）
...
box "RealtimeAgent Server / 现有代码组件" #F3F8E8
participant "RealtimeAgentServer.control_ws\n/ws/control" as ControlWs
participant "RealtimeAgentServer.stream_ws\n/ws/stream" as StreamWs
participant "RealtimeAgentApp\n应用编排" as App
...
participant "VisionModelProvider\n视觉语言模型" as LLM
...
解释这些字段并改变对话状态的是 VisionRealtimeAgentCore。
...
6. `sentence_begin=true` 是 Vision 链路 speech start 的标准来源。
7. `sentence_end=true` 或带 `end_time` 的最终句子是 Vision 链路 speech stop 和 turn commit 的标准来源。
...
| `audio_session.idle_timeout_ms` | VisionRealtimeAgentCore listening 状态空闲多久后关闭连续对话。 |
