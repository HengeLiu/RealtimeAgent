...
文档状态：总体架构设计与历史决策记录。本文覆盖了 `realtime-agent` 从早期设计到老 SDK 对齐阶段的完整架构思路，部分章节保留了历史草案名称和过渡方案。当前可执行代码的开发入口是 `ToolContext` / `TaskContext` 注入的 `ToolDeviceFacade` / `TaskDeviceFacade` typed API，例如 `context.devices.sensors.rgb.one()`、`context.devices.sensors.rgb.stream()`、`context.devices.commands.call()` 和 `context.output.say()`；当前设备注册只接受结构化 `supports`，不接受端侧手写 `routes/subscriptions`；当前 stream 控制事件是 `stream.control.open.requested` / `stream.control.close.requested`，端侧命令事件是 `command.requested` / `command.accepted` / `command.progress` / `command.completed` / `command.failed`。完整 Context API 设计以 [Context 与设备 API 设计说明](../reference/上下文设备接口设计.md) 为准。
...
1. `agent-server/realtime_agent` 是 server-side Python SDK 源码目录，也是 Python 导入包名。
2. `examples` 按项目组织应用、业务能力、参考端侧和应用测试。
3. `agent-server/tests` 和各 `examples/*/tests` 保留 SDK 契约、回放、集成测试。
4. `testdata` 保留跨项目共享的音频样例。
5. `legacy` 保存旧 `openaiglass-sdk`、旧 `openaiglass-external-business` 和历史资料，只作为迁移参考。
...
| Stream Service         | 已实现   | stream chunk 编码、输入/输出 stream 生命周期已由 `agent-server/protocol-tests/sdk/runtime/test_stream_and_audio_pipeline.py` 覆盖。                                                                                                                                                                 |
| Audio Pipeline         | 部分实现 | 已有格式校验、mock provider 和最小音频链路；Vision 路线可通过 `agent-server/protocol-tests/sdk/runtime/test_stream_and_audio_pipeline.py` 做无头回放；完整 wake 后会话生命周期、打断和清理属于 A 线路。                                                                                                                                           |
...
| Memory / Skill / MCP   | 已实现   | C 线路已落地 `MemoryService`、`SkillService`、`McpGateway` 和内置 Tool；由 `agent-server/protocol-tests/sdk/runtime/test_memory_service.py`、`agent-server/protocol-tests/sdk/extensions/test_skill_service.py`、`agent-server/protocol-tests/sdk/extensions/test_mcp_gateway.py`、`agent-server/protocol-tests/acceptance/test_indirect_device_context_contract.py` 覆盖。任何需要设备通讯能力的 Memory / Skill / MCP 都必须封装成 Tool 或 Task。 |
...
| 能力                        | 验收依据                                                                                                                                                                                                                       |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| CLI 和开发者入口            | `agent-server/unit-tests/cli/test_cli_developer_workflow.py`、`agent-server/unit-tests/cli/test_docs_commands.py`                                                                                                                                                      |
...
| 资产缓存与能力回放          | `agent-server/protocol-tests/sdk/runtime/test_stream_and_audio_pipeline.py`                                                                                                                  |
...
| Vision 模型音频闭环           | `agent-server/protocol-tests/sdk/runtime/test_stream_and_audio_pipeline.py`                                                                                                                                                                                     |
| Output Service 与播放仲裁   | `agent-server/protocol-tests/sdk/runtime/test_phase2_providers_output.py`                                                                                                          |
| H 线路迁移样板              | `agent-server/protocol-tests/acceptance/test_auto_discovery_developer_contract.py`、`agent-server/protocol-tests/acceptance/test_no_internal_service_usage_contract.py`                                                                                                                             |
...
input:
  stream_type: "sensor.mic"
  file: "agent-server/model-provider-tests/fixtures/provider/dashscope-nihao-16k.pcm"
  codec: "pcm16le"
  sample_rate: 16000
  channels: 1
assertions:
...
uv run realtime-agent.esp32.start \
  ...
  --project-dir /path/to/external-device-repo/esp32-s3 \
...
legacy/openaiglass-external-business/
agent-server/realtime_agent/
examples/
```

旧业务和旧 SDK 只作为迁移参考。新能力、新文档和新联调入口优先使用根目录下的 `agent-server`、`examples` 和 `docs`。
