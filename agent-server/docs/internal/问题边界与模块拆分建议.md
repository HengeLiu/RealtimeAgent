...
- 后续要做插件市场、设备生态、后台 Tool/Skill 扩展时，缺少稳定的开发者入口。
...
4. **后台 Tool 是一类能力，不是 Agent Core 内部函数**：后台 Tool 需要标准生命周期、可取消、可观测，而不是一段被临时启动的业务代码。
...
| --- | --- | --- | --- |
| 应用产品边界 | 具体用户场景、业务工具、默认配置 | `examples/simple-agent-server` | `app packages` |
| 设备集成边界 | 设备注册、能力声明、控制事件、端侧命令、数据流协议 | `control`、`stream`、`spec`、设备示例 | `device integration platform` |
| 事件与状态边界 | 事件记录、状态投影、跨模块通知、可回放调试 | `runs`、`control-events.jsonl`、`tool-runs`、`agent-events` | `event bus + event log + projectors` |
...
| 上下文边界 | prompt、message、memory、tool schema、Skill 文档、预算裁剪 | `conversation/context`、`prompts`、`memory`、`skills`、`mcp` | `context platform` |
| 能力工具边界 | 短动作能力、工具注册、参数 schema、调用结果 | `tools.py`、`ToolGateway`、应用 `capabilities/tools.py` | `capability/tool platform` |
| 长程任务边界 | 后台 Tool、状态机、反馈、取消、恢复、任务产物 | `tool_run.py`、`task_store`、应用 `capabilities/tool_run.py` | `task/action platform` |
...
package "后台能力（Tool Run）\nlifecycle / feedback / cancel" as BgTool
...
| 项目 | 规则 |
| --- | --- |
| 负责 | 用户场景、业务策略、业务工具、应用默认 prompt、应用配置 |
...
| 禁止依赖 | Agent Core、具体模型 provider、后台 Tool 实现 |
...
| 当前风险 | 已有 `control-events.jsonl`、`agent-events`、`tool-runs`，但事件类型和状态投影还没有成为统一架构中心 |
...
| 拆分目标 | 工具只做短动作；需要持续状态的能力迁到 后台能力（Tool Run） |
...
| 允许依赖 | ToolContext、Capability Platform、Device Platform、Event Platform |
| 禁止依赖 | Agent Core 循环内部状态、provider message 格式 |
| 当前风险 | 当前叫 后台 Tool，但外部契约还不够像 Action，缺少统一 goal/feedback/result/cancel 语义 |
| 拆分目标 | 保留 后台 Tool 命名，但补齐 action-style 生命周期：accepted、running、feedback、succeeded、failed、canceled |
...
| 当前风险 | 模型回复、工具反馈、后台 Tool 通知和端侧播放回执分散时，容易互相打断或覆盖 |
...
| 当前风险 | 开发者需要同时理解设备协议、工具 API、Tool API、prompt 和运行产物，门槛过高 |
| 拆分目标 | 让设备开发者、业务工具开发者、后台 Tool 开发者、Skill/MCP 开发者各自面对一套窄接口 |
...
| P3 | 抽象 Capability Registry：设备能力、工具能力、后台 Tool 能力统一登记但不混同 | 支撑插件和开发者控制台 |
| P4 | 将 后台 Tool 升级为 action-style 生命周期契约 | 支撑长程后台 Tool、取消、恢复、观测 |
| P5 | 建立统一 event bus / event type / state projector | 支撑 reactive agent、自动化、回放调试 |
| P6 | 建立开发者平台：manifest 校验、包结构、运行产物 UI、插件发布流程 | 支撑生态化扩展 |

## 需要立即执行的硬约束

1. `realtime_agent` SDK 核心包不能新增应用后台 Tool fallback。
2. Agent Core 不能直接拼接散落 prompt，必须逐步迁入 `prompts` 和 ContextCompiler。
3. 工具和 后台 Tool 不能直接操作 WebSocket、设备连接对象或 provider SDK 对象。
