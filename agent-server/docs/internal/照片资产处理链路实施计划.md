...
对应设计文档：[SDK 照片资产处理链路设计](照片资产处理链路设计.md)。

本文只记录 SDK 范围的实施计划和当前状态。external-business-app 的自定义视觉功能、自然语言参数提取、360 识别、万物监测和定时任务计划放在 `examples/device_app_demo/docs/`。
...
5. SDK core 只提供照片资产、buffer、claim、appender 和协议能力，不写后台 Tool 逻辑。
...
目标：把 Omni 和 Vision/VL append 差异从 Tool 中抽出。

模块：

```text
agent-server/realtime_agent/conversation/input/
  appender.py
```
...
uv run python -m pytest dev-support/unit-tests dev-support/app-tests -q
```
...
2. 后台 Tool observation 的业务结构由应用层定义。
3. iOS / ESP32 参考端未做真机 metadata 联调。

## 4. 已执行验证

```bash
uv run python -m py_compile agent-server/realtime_agent/asset/photo_asset.py agent-server/realtime_agent/asset/turn_buffer.py agent-server/realtime_agent/asset/service.py agent-server/realtime_agent/tools.py agent-server/realtime_agent/conversation/core/vision_host.py agent-server/realtime_agent/conversation/multimodal/messages.py agent-server/realtime_agent/conversation/multimodal/assets.py agent-server/realtime_agent/config.py agent-server/realtime_agent/app.py
uv run python -m pytest protocol/protocol-tests -q
uv run python -m pytest agent-server/protocol-tests -q
uv run python -m pytest devices/python/protocol-tests -q
uv run python -m pytest dev-support/unit-tests dev-support/app-tests -q
