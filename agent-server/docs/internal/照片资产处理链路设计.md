...
本文描述 `realtime-agent` SDK 内部的照片资产处理链路。它只约束通用 SDK 能力：协议扩展、资产生命周期、turn buffer、消费声明、模型视觉 append 和测试边界；不定义 external-business-app 的 360 识别、万物监测、定时提醒等业务能力。
...
4. 后台 Tool 可以长期订阅或请求视觉资产，但不应该把每一帧都塞进主模型上下文。
...
2. SDK 不统一处理所有视频帧；长期视频流仍可由端侧或应用后台 Tool 自行编排。
...
| server 请求采集 | server | `stream.control.open.requested` -> `/ws/stream` | `capture_photo`、realtime-video、后台 Tool 周期采样。 |
...
```json
{
  ...
  "metadata": {
    ...
    "correlation_id": "turn_or_tool_run_id",
...
| 后台 Tool 运行时分析 | `task_runtime` | 通常否 | 长周期监测、找物、红绿灯。 |
...
后台 Tool 默认不把每帧原图 append 给主模型，只返回 observation、summary 或 Tool Run 回流结果。
...
| `correlation_id` | 否 | 连续采样或 Tool Run 关联 ID。 |
