...
当前代码已经部分落地统一 conversation runtime，但还不是完全统一的单一入口。
...
4. `AudioPipeline` 仍是 app 外层共享的上行音频预处理入口，可在 `audio_pipeline.vad=server_vad` 时输出本地 `speech_started` / `speech_stopped`，但 conversation runtime 内部的 turn 边界由 `SpeechInputBoundary` 适配。
...
2. `AudioPipeline` 与旧 `RealtimeAudioNormalizer` 的职责重叠已经被标记为 legacy-only，但旧文件尚未删除。
3. app 层仍通过 `_handle_pipeline_event()` 兼容处理 legacy pipeline event 和 conversation runtime event，后续需要把命名收敛为更中性的 runtime event handler。
4. 打断逻辑已经从 VAD 中移出，但仍分布在 Agent Core、conversation runtime 和 `OutputService` 适配器中，后续可以进一步收敛边界。
...
| `OutputInterruptionController` | 执行取消模型响应、取消 output stream、清理未播放音频等打断动作。当前仍由 `OutputService`、Pipeline bridge 和 Agent Core 分散承担。 | 共享目标 |
...
3. 如果可打断，发布 `output_cancel_requested`。
