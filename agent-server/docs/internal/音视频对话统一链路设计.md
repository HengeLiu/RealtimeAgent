...
5. Vision / VL 和 Omni Manual conversation runtime 均依赖 Paraformer 这类 ASR/VAD 合一 provider：同一个实时 ASR 事件里既有转写文本，也有 `sentence_begin` / `sentence_end` 边界字段。
6. 两条 conversation 链路均使用 `AsrSpeechInputBoundary` 将 ASR partial、sentence begin、sentence end/final 映射为 `SpeechInputDelta`。
7. Omni conversation runtime 只支持 Manual 模式，Omni provider `turn_detection` 固定为 `manual`；speech boundary 由 ASR provider 句边界产生，不再保留 RMS VAD 版 Omni 链路。
...
3. ASR-backed speech boundary 只输出 `speech_started` 和 `speech_stopped`，不做打断、不做视觉采样、不提交模型；ASR 文本由 `SpeechInputBoundary` 以 `asr_text_delta/final_text` 输出。
...
| `AsrSpeechInputBoundary` | 包装实时 ASR provider，输出 `audio_chunk`、`asr_text_delta`、`turn_started`、`turn_ended(final_text)`。 | 共享 |
| `ASRProvider` | 输出实时 ASR 文本增量、final_text 和句子边界。当前 VL 与 Omni Manual conversation runtime 共享该输入边界来源。 | 共享 |
...
`AsrSpeechInputBoundary` 是当前 conversation runtime 的共享输入边界组件。
...
7. Omni provider 不再负责输出 `speech_started` / `speech_stopped`，相关端侧事件由 ASR-backed speech boundary 统一产生。
...
2. 为 VL 和 Omni Manual 共同提供 `paraformer_boundary` 适配器，把 ASR/VAD 合一事件转换成统一 speech 事件。
3. 保留 sentence id 去重和事件顺序校验。
4. 让 VL 和 Omni Manual 都消费同一套 ASR-backed speech 事件。
...
1. VL 和 Omni Manual 均由同一 ASR-backed `SpeechInputBoundary` 产出 `speech_started` / `speech_stopped`。
2. ASR-backed speech boundary 不输出打断事件。
