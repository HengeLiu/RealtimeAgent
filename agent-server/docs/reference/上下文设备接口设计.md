...
| `query_route_plan` | 通过 AMap MCP 查询步行路线；会把地址先解析成经纬度。 | 优先使用应用 MCP 配置；未配置时读取 `AMAP_MCP_URL`、`AMAP_MCP_BEARER_TOKEN`、`AMAP_MCP_API_KEY`。 |
...
其中 `timer_task` 是 SDK 内置 Task，用于倒计时、稍后提醒和到点提示；其它业务 Task 由应用注册后自动生成对应启动工具。
