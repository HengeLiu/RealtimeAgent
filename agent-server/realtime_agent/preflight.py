...
def main(argv: list[str] | None = None) -> None:
    ...
    loaded = load_yaml_config(args.config) if args.config else RealtimeAgentYamlConfig()
    ...
    checks.append(_vision_multimodal_config_check(loaded))
...
def live_check(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--config", default="examples/for-blind-app/agent-server/server.yaml")
    parser.add_argument("--generated-dir", default="examples/for-blind-app/agent-server/config/generated")
    ...
    checks = [
        ...
        _vision_multimodal_config_check(loaded),
...
def _config_validation_check(config: RealtimeAgentYamlConfig) -> dict:
    ...
    if config.agent.mode not in {"vision", "omni"}:
...
def _boundary_check() -> dict:
    ...
    try:
        ...
        errors.extend(f"endpoint reference leaked from realtime_agent: {name}" for name in leaked)
    except Exception as exc:
        errors.append(f"{type(exc).__name__}: {exc}")
    return {"name": "boundary", "ok": not errors, "errors": errors}


def _provider_key_check(config: RealtimeAgentYamlConfig) -> dict:
    ...
    if (
        vision.asr_provider == "dashscope"
        or vision.provider in {"dashscope", "dashscope-compatible"}
        or vision.tts_provider == "dashscope"
    ):
    ...
    if missing and allow_fallback:
        ...
        errors.append(f"missing provider keys {missing}; set key or enable agent.vision.allow_mock_fallback")
    return {
        "name": "provider_keys",
        "ok": not errors,
...
def _provider_runtime_profile_check(config: RealtimeAgentYamlConfig) -> dict:
    ...
    real_provider_requested = any(
        ...
        for provider in (vision.asr_provider, vision.provider, vision.tts_provider)
    ) or (config.agent.mode == "omni" and config.agent.omni.provider not in {"mock", ""})
    if not real_provider_requested:
        profile = "mock"
    elif key_check["missing_env"] and vision.allow_mock_fallback:
    ...
    return {
        ...
        "profile": profile,
        "providers": {
            "asr": vision.asr_provider,
            "vision": vision.provider,
            "tts": vision.tts_provider,
            "omni": config.agent.omni.provider if config.agent.mode == "omni" else "",
        },
        "timeout_seconds": vision.request_timeout_seconds,
...
def _vision_multimodal_config_check(config: RealtimeAgentYamlConfig) -> dict:
    """检查 Vision 多模态配置是否与 provider/model 基本匹配。

    主要逻辑：多模态启用时要求 Vision provider 是 Chat Completions 兼容形态，并使用
    ...
    if not multimodal.enabled:
        return {"name": "vision_multimodal", "ok": True, "enabled": False}
    errors: list[str] = []
    warnings: list[str] = []
    if vision.provider not in {"dashscope-compatible", "openai-compatible"}:
        errors.append(f"vision multimodal requires chat-compatible provider, got {vision.provider}")
...
def _mcp_config_check(config: RealtimeAgentYamlConfig) -> dict:
    """检查 MCP 配置文件和外部 server smoke 状态。"""

    path = _resolve_realtime_agent_path(config.mcp.config_path)
...
def _memory_skill_config_check(config: RealtimeAgentYamlConfig) -> dict:
    """检查 Memory store 和 Skill roots 的配置可解释性。"""

    errors: list[str] = []
    degradations: list[str] = []
    memory_path = _resolve_realtime_agent_path(config.memory.path)
    ...
    skill_roots = [_resolve_realtime_agent_path(root) for root in config.skill.roots]
...
def _endpoint_config_check(config: RealtimeAgentYamlConfig) -> dict:
...
def _audio_pipeline_check(config: RealtimeAgentYamlConfig) -> dict:
    ...
    if audio.vad in {"", "disabled", "off", "false"}:
        ...
        degradations.append("audio_pipeline.vad uses server-side VAD for Vision realtime speech boundaries")
...
def _live_server_check(config: RealtimeAgentYamlConfig) -> dict:
    ...
    return {
        ...
        "action": "" if not errors else f"启动 server: uv run realtime-agent.server.run --config <server.yaml>，当前检查地址 {config.server.public_url}",
        "errors": errors,
    }
...
def _recent_playback_check() -> dict:
    ...
    roots = [Path("runs"), Path("realtime-agent/runs")]
...
def _recent_playback_observation(config: RealtimeAgentYamlConfig) -> dict:
    ...
    return {
        **result,
        "ok": True,
        "observed": False,
        "action": f"运行一次 playback: uv run realtime-agent.config.sync --output-dir {generated_dir} 后使用生成的 glass.playback.yaml",
    }


def _recent_registration_failures_check(config: RealtimeAgentYamlConfig) -> dict:
    """扫描最近控制事件中的注册失败。
...
def _reference_endpoint_config_consistency_check(generated_dir: Path, config: RealtimeAgentYamlConfig) -> dict:
    """检查 config.sync 生成的多端配置是否与 server 配置一致。

    参数：`generated_dir` 为 `realtime-agent.config.sync` 输出目录。
    返回值：结构化检查结果。
    异常情况：本函数只记录错误，不抛出异常。
    """
    ...
    if missing:
        return {
            ...
            "action": f"先运行 realtime-agent.config.sync --output-dir {generated_dir}",
        }
    ...
    try:
        ...
        if esp32_values.get("REALTIME_AGENT_SERVER_URL") != expected_server_url:
            errors.append("esp32_s3 REALTIME_AGENT_SERVER_URL differs from live-check config")
        user_ids.add(str(esp32_values.get("REALTIME_AGENT_USER_ID", "")))
        device_ids.add(str(esp32_values.get("REALTIME_AGENT_DEVICE_ID", "")))
    except Exception as exc:
        errors.append(f"{type(exc).__name__}: {exc}")
...
def _resolve_realtime_agent_path(raw_path: str) -> Path:
    path = Path(raw_path)
    if path.exists():
        return path
    if path.parts and path.parts[0] == "realtime-agent":
