...
def package_check(argv: list[str] | None = None) -> None:
    """检查 realtime-agent SDK 包公共面。

    主要逻辑：
    1. 校验 pyproject 中的 entry point 能导入。
    2. 校验 README 常用公开类能从 realtime_agent 顶层导入。
    3. 可选写入 JSON 报告。
    """

    parser = argparse.ArgumentParser(prog="realtime-agent.sdk.package-check", description="检查 realtime-agent SDK 包")
    ...
    endpoint_leaks = [
        name
        for name in ("NetworkPythonPlaybackEndpoint", "PythonPhoneMockEndpoint", "RealtimeAgentWebEndpoint")
        if hasattr(package, name)
    ]
    errors.extend(f"endpoint reference leaked from realtime_agent: {name}" for name in endpoint_leaks)
    ...
    if args.skip_wheel_build:
        ...
        with tempfile.TemporaryDirectory(prefix="realtime-agent-release-") as temp_dir:
...
def _release_candidate_check(*, version: str, audio_root: Path) -> dict:
    ...
    if not changelog.exists():
        ...
        for expected in (version, "架构变更点", "package-check", "Context API"):
...
def _wheel_build_check(audio_root: Path, out_dir: Path) -> dict:
    ...
    参数：`audio_root` 为 realtime-agent 项目根目录。
    返回值：结构化检查结果。
...
def _wheel_install_check(wheel_path: Path, public_names: list[str]) -> dict:
    ...
    code = (
        ...
        f"missing=[name for name in {public_names!r} if not hasattr(realtime_agent, name)]; "
...
def _wheel_contents_check(wheel_path: Path) -> dict:
    ...
    required = ["realtime_agent/py.typed"]
...
def _editable_install_check(audio_root: Path, public_names: list[str]) -> dict:
    ...
    实际导入 `realtime_agent` 并确认公开对象存在。
    参数：`audio_root` 为 realtime-agent 项目根目录，`public_names` 为必须导出的对象。
    返回值：结构化检查结果。
    异常情况：命令失败时记录错误。
    """
...
def _endpoint_source_check(audio_root: Path) -> dict:
    ...
    主要逻辑：端侧参考实现不进入 `realtime_agent` SDK 内部命名空间，但 release
...
def _source_boundary_check(audio_root: Path) -> dict:
    ...
    主要逻辑：`realtime_agent.cli` 可以知道参考端侧位置；
    核心包、Tool / Task / Agent / Stream / Output 服务不能 import examples 或端侧工程。
    """

    core_root = audio_root / "agent-server" / "realtime_agent"
