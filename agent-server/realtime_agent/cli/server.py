...
def start(argv: list[str] | None = None) -> None:
    ...
    2. 非 dry-run 时使用当前 Python 解释器启动 `realtime_agent.server.main`。
    ...
    parser = argparse.ArgumentParser(prog="realtime-agent.server.start", description="后台启动 realtime-agent server")
    parser.add_argument("--config", default="")
    parser.add_argument("--app-name", default="", help="应用名称，对应 examples/<app-name>/agent-server")
    ...
    if args.dry_run:
        ...
        log_file.write_text("realtime-agent server dry-run start\n", encoding="utf-8")
    ...
    command = [
        sys.executable,
        "-c",
        "from realtime_agent.server import main; main()",
    ]
    if args.app_name:
        command.extend(["--app-name", args.app_name, "--app-root", args.app_root])
    else:
        command.extend(["--config", args.config or "examples/device_app_demo/agent-server/server.yaml"])
    if args.app_module:
...
def stop(argv: list[str] | None = None) -> None:
    """停止由 `realtime-agent.server.start` 启动的 server。"""

    parser = argparse.ArgumentParser(prog="realtime-agent.server.stop", description="停止 realtime-agent server")
    parser.add_argument("--pid-file", default="runs/default-app/server.pid")
...
def logs(argv: list[str] | None = None) -> None:
    ...
    parser = argparse.ArgumentParser(prog="realtime-agent.server.logs", description="查看 realtime-agent server 日志")
    parser.add_argument("--log-file", default="runs/default-app/server.log")
...
def _resolve_launch_metadata(args: argparse.Namespace):
    ...
    参数：`args` 为命令行参数。
    返回值：`RealtimeAgentConfig` 和 `AppLaunch`。
    异常情况：配置缺失、非法路径或非法 app-name 时由 app_loader 抛出。
    """

    if args.app_name:
        return load_app_config(args.app_name, app_root=args.app_root)
    return load_config_as_app(args.config or "examples/device_app_demo/agent-server/server.yaml")
