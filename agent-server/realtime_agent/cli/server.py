...
def start(argv: list[str] | None = None) -> None:
    ...
    if args.app_name:
        ...
        command.extend(["--config", args.config or "examples/simple-agent-server/server.yaml"])
    if args.app_module:
...
def _resolve_launch_metadata(args: argparse.Namespace):
    ...
    return load_config_as_app(args.config or "examples/simple-agent-server/server.yaml")
