...
class OutputInterruptionController:
    ...
    def _is_server_vad_boundary(*, reason: str, diagnostics: Mapping[str, Any]) -> bool:
        ...
        主要逻辑：Omni Manual 链路使用 `conversation_vad_*`，旧 AudioPipeline 使用
        `server_vad_*`；ASR-backed VL 边界会带 `asr_boundary`，不走该保护。
        ...
        if diagnostics.get("asr_boundary"):
            return False
        return reason in {"conversation_vad_speech_started", "server_vad_speech_started"}
