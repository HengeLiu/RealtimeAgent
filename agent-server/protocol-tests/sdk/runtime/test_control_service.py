...
def _supports_for_routes(routes: list[dict]) -> dict:
    ...
    for route in routes:
        ...
        if stream_type == "sensor.rgb":
            sensors.append({"type": "rgb", "modes": ["single", "continuous"]})
        if stream_type == "actuator.haptic":
            actuators.append({"type": "vibrator"})
        command = (route.get("filter") or {}).get("payload.command")
        if command == "haptic.vibrate":
            actuators.append({"type": "vibrator"})
    return {"sensors": sensors, "actuators": actuators}


def _properties_for_routes(routes: list[dict]) -> dict:
    """把系统音频和命令端点意图映射为公开 properties。"""
    ...
    for route in routes:
        ...
        if route.get("event") == "command.*":
            properties["peer.video.receiver"] = True
    return properties
...
def _registration(device_id: str, routes: list[dict]) -> Event:
    return Event(
        ...
        payload={
            ...
            "auth": {"mode": "disabled"},
            "properties": _properties_for_routes(routes),
...
def test_registration_validator_applies_route_config() -> None:
    ...
    wildcard = _registration("dev-wild", [])
    wildcard.payload["routes"] = [{"event": "*"}]
    wildcard = service.register_device(wildcard, FakeConnection("dev-wild"))
...
def test_route_filter_matches_envelope_payload_and_arrays() -> None:
    """测试目标：验证公开 supports 编译出的命令 filter 能匹配 payload 字段。

    测试方法：注册一个 vibrator 执行器，再发布 `haptic.vibrate` 命令。
    预期结果：只有符合 `payload.command` filter 的设备收到事件。
    """
    ...
    assert [event.payload["command"] for event in endpoint.events] == ["haptic.vibrate"]
...
def test_route_filter_rejects_regex_and_unknown_paths() -> None:
    """测试目标：明确公开注册协议不允许端侧手写内部 routes。

    测试方法：注册 payload 中直接携带 routes。
    预期结果：注册失败，并在 debug snapshot 中保留失败原因。
    """

    service = ControlService()
    regex_event = _registration("dev-regex", [])
    regex_event.payload["routes"] = [{"event": "stream.output.*", "filter": {"payload.name|regex": "x"}}]
    unknown_event = _registration("dev-unknown", [])
    unknown_event.payload["routes"] = [{"event": "stream.output.*", "filter": {"device_id": "dev-001"}}]
    regex = service.register_device(regex_event, FakeConnection("dev-regex"))
    unknown = service.register_device(unknown_event, FakeConnection("dev-unknown"))

    assert regex.event_name == "control.device.register.failed"
    assert unknown.event_name == "control.device.register.failed"
    assert "must not contain routes" in service.build_device_snapshot("dev-regex")["register_failed_reason"]
