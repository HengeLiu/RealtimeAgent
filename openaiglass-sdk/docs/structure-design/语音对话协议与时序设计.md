...
本文档是半双工语音对话的基线设计：播放期间暂停麦克风和 WakeNet，用户不能在播报中自然插话。

全双工实时语音对话已经拆为独立专项设计，见 [全双工实时语音对话设计](./全双工实时语音对话设计.md)。后续 SDK 需要同时保留半双工和全双工两种会话模式，功能开发者通过 SDK 配置选择，不应在业务代码里自行改写语音协议。
...
- 当前轮用户语音不会直接送入对话模型，而是先通过独立 ASR 转成文本，再以文本多轮消息调用对话模型
...
8. `VoiceSessionController` 聚合该轮音频，并调用独立 ASR 服务把音频转写成文本。
9. `VoiceSessionController` 使用“历史文本 + 当前轮 ASR 文本”构造多轮 `messages`，再调用 `qwen3.5-omni-plus`。
10. 服务端一边接收模型流式音频输出，一边向眼镜下发 `actuator.audio.play` 并持续回推回复音频流。
11. 回复播放期间，眼镜暂停 WakeNet 和麦克风上行。
12. 眼镜侧 `actuator-hub` 播放完毕后回 `actuator.audio.finished`，随后恢复待命监听。
...
- 服务端在进入对话模型前先做独立 ASR，把每轮用户语音转成稳定文本
...
第一期实际实现中，服务端已引入独立 ASR 调用。

约束如下：

1. 用户语音上行链路保持不变，仍然是 `/ws_audio + audio_chunk`。
2. `sensor.audio.segment.finished` 到达后，服务端必须先把聚合音频送入 ASR。
3. 只有当 ASR 成功返回文本后，才允许进入对话模型调用。
4. 多轮上下文的主载体应是文本，而不是反复回放历史用户音频。
5. 当前轮原始音频仍需要落盘保存，但它的主要作用是审计、复盘和必要时重做转写，不是直接充当多轮历史主输入。
...
对应的握手注册过程见 [设备握手与注册协议设计.md](./设备握手与注册协议设计.md)。
...
详细帧格式见 [媒体流传输格式设计.md](./媒体流传输格式设计.md)。
...
`sensor.audio.segment.finished` 到达后，服务端会进入一个独立的 ASR 阶段。

建议职责：

1. 把当前 `segment_id` 的聚合音频封装为完整 `wav`。
2. 调用百炼独立 ASR 模型，例如 `qwen3-asr-flash`。
3. 生成当前轮用户文本，并挂接为本轮 `MessageContext.text`。
4. 把转写结果落成 `DerivedArtifact`，便于后续审计、回放和问题排查。
...
2. 先通过阿里云百炼独立 ASR 把该轮语音转成文本，例如调用 `qwen3-asr-flash`
3. 用“历史文本 + 当前轮 ASR 文本”构造多轮 `messages`
4. 再通过阿里云百炼 OpenAI 兼容接口调用 `qwen3.5-omni-plus`
5. 使用流式响应，不能等整轮模型结果结束后再下发播放
6. 请求输出模态设置为 `["text","audio"]`
7. 音频输出配置为 `audio={"voice":"Cherry","format":"wav"}`
8. 思考模式关闭，因为 `Qwen3-Omni-Flash` 在思考模式下不支持输出音频
...
1. ASR 请求不应混入额外对话提示词。
2. ASR 阶段的目标只有一个：稳定产出用户文本。
3. 当前轮原始音频在进入 ASR 前必须先落盘。
4. 当前轮 ASR 文本需要进入 `MessageContext`，作为下一轮历史输入。
...
- 语音输入在服务端聚合后先进入独立 ASR，而不是直接送给对话模型
- 对话模型多轮历史应优先使用用户文本和助手文本
- 当前轮原始音频、ASR 转写文本和最终回复文本都应进入日志和会话上下文
...
participant "qwen3-asr-flash" as ASR
participant "qwen3.5-omni-plus" as LLM
...
CTRL -> VS : 生成当前轮 wav
VS -> ASR : 非流式 ASR 调用
ASR --> VS : transcript text
CTRL -> VS : 构造文本多轮 messages
VS -> LLM : stream=true 调用 qwen3.5-omni-plus
LLM --> VS : 文本增量 + 音频增量
VS -> CTRL : 首段可播放音频
