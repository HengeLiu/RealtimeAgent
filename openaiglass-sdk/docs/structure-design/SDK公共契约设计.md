...
本文档用于说明当前 SDK 已经对业务开发者和三端运行时暴露的公共契约。旧的第二期阶段计划已经删除，具体演进过程见 [stage2 迭代记录](../stage2/)。
...
1. 确认当前 `find_object` 样例和回放资产在公共契约调整后仍可运行。
...
5. 公共契约修改能够被测试发现。
6. `sdk_boundary` 预检通过，证明公共契约之外的系统运行时没有反向依赖官方样例，也没有重新内建具体业务能力。
