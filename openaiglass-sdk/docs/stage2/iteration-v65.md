...
功能开发团队需要 Agent 能主动记住用户的稳定偏好、基本信息和行为习惯，也要允许用户通过自然语言主动新增、更新或删除记忆。SDK 已在 `sdk-v50` 提供冷热记忆、`memory_search` 和 `manage_memory`，但无模型兜底路径对“忘掉刚才那条记忆”“删除我的导航偏好”等自然语言控制不够稳。
...
3. 删除流程增加多级兜底：先按 `memory_id`，再按标题和类型，再按标题不限定类型，最后按原始自然语言查询匹配。
