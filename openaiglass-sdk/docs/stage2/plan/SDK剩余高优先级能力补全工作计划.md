...
4. 分布式任务持久化：补多进程、多实例、数据库级任务平台；第一版优先使用 SQLite 文件库和内存库。已在 sdk-v17 完成 SQLite 单机多进程第一版。
...
状态：已完成第一版，落地版本 sdk-v17。

主要开发任务：

1. 新增 `SQLiteTaskPersistenceStore`，使用 Python 标准库 `sqlite3`。已完成。
2. 设计并创建 `tasks`、`task_events`、`task_leases`、`schema_migrations` 表。已完成。
3. 支持事件幂等、任务快照恢复、终态清理和单机多进程 WAL。已完成文件库 WAL、事件表幂等、快照恢复、租约抢占；终态清理沿用 manager 保存契约。
4. `TaskRuntimeManager` 支持通过配置选择内存、文件或 SQLite 存储。已完成 `enable_sqlite_persistence(...)`。

测试要求：

1. 单元测试覆盖建表、写任务、写事件、重复事件幂等、重启恢复。已完成。
2. 多 manager 使用同一个 SQLite 文件时，租约能阻止同一任务被重复恢复执行。已完成 store 级租约测试。
3. 文件型存储继续兼容，不因 SQLite 引入而失效。已通过全量单测。
...
状态：sdk-v17 第一版已通过自动化验收。

通过标准：

1. 服务重启后任务状态、事件日志和幂等记录仍可恢复。已完成。
2. 单机多进程同时访问同一 SQLite 文件时，任务租约行为可预测。已完成。
3. SQLite 存储失败时有结构化错误，业务 Task 不依赖具体数据库实现。已完成存储契约，业务 Task 不直接接触 SQLite。
