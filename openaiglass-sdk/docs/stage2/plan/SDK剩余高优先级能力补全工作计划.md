...
1. 手机视觉执行框架的资源管理：真 iOS 运行时补统一模型资源池、任务抢占和功耗治理。
2. 完整播放仲裁和用户语音打断：统一普通 Agent 回复、任务通知、视觉告警和用户语音打断。
3. 账号权限、组织管理和远程配置中心：补完整授权、审计、组织树和远程配置中心。
...
1. 在服务端引入统一 `PlaybackArbiter`，把 Agent 回复、任务通知、视觉告警都转成 `PlaybackIntent`。
2. 把 `NotificationCoordinator` 收敛为仲裁输入或兼容层，不再成为唯一播放策略入口。
3. 新增用户打断控制事件，先支持“停止当前播报并清空或保留队列”的半双工语义。
4. 在运行态快照输出当前播放 lease、等待队列、最近仲裁决策和最近用户打断。

测试要求：

1. 单元测试覆盖低优先级排队、高优先级抢播、用户打断、打断后恢复策略。
2. 回放或语音样例测试覆盖普通 Agent 回复与任务通知的顺序不互相污染。
3. DEBUG 日志可以解释每次播放决策。

### 4.3 迭代 C：账号权限、组织管理和配置 Provider

主要开发任务：

1. 在 SDK 中新增账号目录和组织树模型，支持默认本地账号、组织节点、设备组和角色绑定。
2. 增加最小权限策略：设备注册、跨账号绑定、Task 访问设备能力、Tool 调用前检查权限。
3. 增加审计事件输出，先支持内存和文件 sink。
4. 增加远程配置 Provider 抽象，先落地内存 Provider 和文件 Provider，预留 HTTP Provider。

测试要求：

1. 本地默认账号不增加普通开发启动成本。
2. 跨组织访问被拒绝并记录审计事件。
3. 配置 Provider 的配置值变更可以被 SDK 读取，并能在运行态快照中看到配置版本。
...
1. 普通 Agent 回复、任务通知和视觉告警都经过同一仲裁入口。
2. 用户打断事件可以终止当前播报，并按策略处理队列。
3. 运行态快照可以解释当前播报为什么被播放、排队、抢播、恢复或丢弃。
...
1. 本地单账号开发无需额外配置即可跑通。
2. 跨账号或跨组织访问会被拒绝并记录审计事件。
3. 配置 Provider 可以在不改业务代码的情况下调整 SDK 策略参数。
