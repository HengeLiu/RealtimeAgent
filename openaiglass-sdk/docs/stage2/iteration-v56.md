...
6. 播放任务改为 `xTaskCreateWithCaps(..., MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT)`，并在 `sdkconfig.defaults` 中启用 `CONFIG_SPIRAM_ALLOW_STACK_EXTERNAL_MEMORY=y`，避免 AEC 高性能模式占用内部 RAM 后播放任务无法创建；失败日志会输出是否允许外部栈、internal/PSRAM 的可用和最大连续块。
...
4. 如果设备目录已有旧 `sdkconfig`，需要执行 `idf.py reconfigure` 或删除旧 `sdkconfig` 后重新配置，确保 `CONFIG_SPIRAM_ALLOW_STACK_EXTERNAL_MEMORY=y` 真正进入本次构建。
