...
DEFAULT_SERVER_CONFIG = REPO_ROOT / "config/local_server.env"
DEFAULT_PREFLIGHT_REPORT = REPO_ROOT / "logs/sdk-preflight-current.json"
PHONE_APP_CONFIG = REPO_ROOT / "phone/ios/GlassesVideoReceiver/AppConfig.plist"
PHONE_PROJECT = REPO_ROOT / "phone/ios/GlassesVideoReceiver.xcodeproj"
GLASS_PROJECT = REPO_ROOT / "glass/src"
GLASS_KCONFIG = REPO_ROOT / "glass/src/main/Kconfig.projbuild"
GLASS_LOCAL_CONFIG = REPO_ROOT / "glass/config/local_build.env"
...
def check_paths() -> LiveCheckResult:
    ...
    start = perf_counter()
    paths = {
        "server_script": REPO_ROOT / "script/run_server.sh",
        "sdk_preflight_script": REPO_ROOT / "script/run_sdk_preflight.py",
    ...
    if not report_path.exists():
        return LiveCheckResult(
            ...
            details={
                ...
                "message": "未找到预检报告，请先执行 script/run_sdk_preflight.py",
            },
        )
    try:
...
def check_config_alignment(server_config_path: Path) -> LiveCheckResult:
    ...
    if not server_config_path.exists():
        failures.append("缺少 config/local_server.env")
    if not token_map:
    ...
    if expected_glass_ws and glass_server_ws_uri != expected_glass_ws:
        failures.append("眼镜 glass/config/local_build.env 中 GLASS_SERVER_WS_URI 与 SERVER_PUBLIC_HOST/PORT 不一致")
