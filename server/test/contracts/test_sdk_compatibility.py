...
from example.server.main import create_sdk
from openaiglasses.testing import ScenarioRunner


ROOT = Path(__file__).resolve().parents[3]
COMPAT_ROOT = ROOT / "testdata/compat"
...
def test_find_object_compat_suite_validates_all_scenarios() -> None:
    """测试目标：验证第二期官方样例兼容性场景都能通过 manifest 校验。
    ...
    for scenario in suite["scenarios"]:
        sdk = create_sdk()
        runner = ScenarioRunner(sdk, workspace_root=ROOT)
        validation = runner.validate(ROOT / str(scenario))
...
def test_find_object_compat_suite_executes_all_scenarios() -> None:
    """测试目标：验证第二期官方样例兼容性场景在当前 SDK 中仍可执行。
    ...
    for scenario in suite["scenarios"]:
        sdk = create_sdk()
        runner = ScenarioRunner(sdk, workspace_root=ROOT)
        result = runner.run(ROOT / str(scenario))
