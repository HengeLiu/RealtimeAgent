...
def test_sdk_registry_can_load_official_example() -> None:
    """测试目标：验证唯一官方 example 可注册到 SDK。

    测试方法：
    1. 调用 `example.server.main.create_sdk()` 创建 SDK。
...
def test_example_tool_can_create_managed_task() -> None:
    """测试目标：验证官方 example 的 Tool 会通过 SDK 真正创建任务。
...
def test_official_example_replay_can_complete_find_object() -> None:
    """测试目标：验证官方 example 可以通过回放完成找物体闭环。

    测试方法：
    1. 创建已注册示例能力的 SDK。
    2. 使用 `ScenarioRunner` 读取 `example/scenario/find_object_basic.json`。
...
def test_scenario_runner_can_load_frames_from_testdata_manifest() -> None:
    ...
    sdk = create_sdk()
    result = ScenarioRunner(sdk).run(ROOT / "testdata/scenario/find_object_with_testdata.json")
...
def test_scenario_runner_can_replay_cancelled_find_object_manifest() -> None:
    ...
    sdk = create_sdk()
    result = ScenarioRunner(sdk).run(ROOT / "testdata/scenario/find_object_cancelled.json")
...
def test_scenario_runner_can_report_missing_phone_failure() -> None:
    ...
    sdk = create_sdk()
    result = ScenarioRunner(sdk).run(ROOT / "testdata/scenario/find_object_missing_phone.json")
...
def test_scenario_runner_can_report_video_link_start_failure() -> None:
    ...
    sdk = create_sdk()
    result = ScenarioRunner(sdk).run(ROOT / "testdata/scenario/find_object_video_link_start_failed.json")
...
def test_example_server_handle_can_build_real_runtime() -> None:
    """测试目标：验证官方 example 已可构建真实服务端句柄。

    测试方法：
    1. 调用 `example.server.main.create_server_handle(...)`。
...
def test_example_server_handle_can_start_real_http_service() -> None:
    """测试目标：验证官方 example 服务端句柄可真实启动 HTTP 服务。

    测试方法：
    1. 构建并启动 `example` 服务端句柄。
    2. 请求 `/api/health`。
...
def test_sdk_package_does_not_import_example() -> None:
    """测试目标：验证 SDK 不反向依赖 example。
    ...
    预期结果：
    1. SDK 源码中不存在对 `example` 的导入。
    """

    sdk_root = ROOT / "sdk/python/openaiglasses"
    offenders: list[str] = []
    for path in sdk_root.rglob("*.py"):
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                ...
                continue
            if any(name == "example" or name.startswith("example.") for name in names):
