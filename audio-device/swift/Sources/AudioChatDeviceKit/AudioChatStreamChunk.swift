...
public struct AudioChatStreamChunk {
...
public enum AudioChatStreamChunkCodec {
    public static func encode(_ chunk: AudioChatStreamChunk) throws -> Data {
    ...
    public static func decode(_ data: Data) throws -> AudioChatStreamChunk {
        guard data.count >= 4 else {
            throw AudioChatDeviceError.invalidStreamChunk("message too short")
        }
        ...
        guard headerLength > 0, headerEnd <= data.count else {
            throw AudioChatDeviceError.invalidStreamChunk("invalid header length")
        }
        ...
        guard let header = try JSONSerialization.jsonObject(with: headerData) as? [String: Any] else {
            throw AudioChatDeviceError.invalidStreamChunk("header is not json object")
        }
        let payloadSize = header["payload_size"] as? Int ?? -1
        guard payloadSize == payload.count else {
            throw AudioChatDeviceError.invalidStreamChunk("payload_size mismatch")
        }
    ...
    private static func string(_ header: [String: Any], _ key: String) throws -> String {
        guard let value = header[key] as? String else {
            throw AudioChatDeviceError.invalidStreamChunk("missing \(key)")
        }
        return value
    }
