...
from audio_chat_device import StreamChunk, StreamChunkCodec
...
def test_python_device_stream_codec_reads_golden_fixture() -> None:
    ...
    测试方法：解码 `testdata/protocol/streams/rgb-chunk.bin` 并比对 header。
    预期结果：SDK 编解码结果与协议样例一致。
    """

    header = json.loads((ROOT / "testdata/protocol/streams/rgb-header.json").read_text(encoding="utf-8"))
    raw = (ROOT / "testdata/protocol/streams/rgb-chunk.bin").read_bytes()
