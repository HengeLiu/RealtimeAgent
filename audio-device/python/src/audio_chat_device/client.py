...
from .events import AudioChatEvent
from .stream import StreamChunk, StreamChunkCodec

EventHandler = Callable[[AudioChatEvent], Awaitable[None]]
...
class CommandResponder:
    ...
    def __init__(self, client: "AudioChatDeviceClient", request: AudioChatEvent) -> None:
        self.client = client
...
class AudioChatDeviceClient:
    ...
    async def register(self, *, start_heartbeat: bool = True) -> AudioChatEvent:
        """发送注册事件并等待注册结果。
    ...
    def event(self, event_name: str, payload: dict[str, Any] | None = None, **extra: Any) -> AudioChatEvent:
        """构造当前设备生产的控制事件。"""
    ...
    async def send_event(self, event: AudioChatEvent) -> None:
        """发送控制事件。"""
    ...
    async def receive_event(self, *, timeout: float | None = None) -> AudioChatEvent:
        """读取一个控制事件。"""
        ...
        event = AudioChatEvent.from_json(message.data)
    ...
    async def dispatch_event(self, event: AudioChatEvent) -> bool:
        """按已注册回调分发事件。
