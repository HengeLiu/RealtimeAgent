...
sdk-v14 已经可以支撑第一期中“非实时语音交互、普通文本流式 TTS 透传、通知仲裁与高优先级通知抢播、AgentCore 工具调用、Tool/MCP 接入、最小 Skill Runtime、抓拍图片理解、SDK 托管 Task、任务文件型自动持久化、眼镜与手机注册绑定、账号级设备组织、手机视频链路任务最小闭环、手机视觉资源策略、真 iOS 视觉资源协调、带断言的音频样例回放测试、iOS 与 ESP32 端侧源码包清单和 package-check 校验”等主要基础能力。

当前用户重新排序后的 SDK 后续推进范围剩余三项：完整播放仲裁和用户语音打断、账号权限/组织管理/远程配置中心、SQLite 优先的分布式任务持久化。真实地图服务适配治理、导航任务通用模板、Skill Runtime 产品化治理、设备级回放断言增强、iOS/ESP32 二进制或组件仓库发布暂时不纳入下一轮开发。
...
1. 以独立迭代实现完整播放仲裁、用户打断、播放中断和多源提示仲裁。
2. 补齐账号权限、组织树、审计事件和远程配置 Provider。
3. 把单机任务持久化升级为 SQLite 优先的任务平台，支持数据库存储、单机多进程租约和服务重启恢复。
4. 后续暂缓项包括导航类长任务模板、地图 MCP Adapter 模板、设备级回放断言增强、Skill Runtime 产品化治理、iOS/ESP32 二进制发布。
