...
sdk-v8 已经可以支撑第一期中“非实时语音交互、普通文本流式 TTS 透传、通知仲裁与高优先级通知抢播、AgentCore 工具调用、Tool/MCP 接入、抓拍图片理解、SDK 托管 Task、眼镜与手机注册绑定、手机视频链路任务最小闭环、手机视觉资源策略最小闭环、设备级回放测试”等主要基础能力。
...
| 眼镜、手机、服务器设备组绑定 | 第 1 项和第 9 项绑定关系；Phase B/I | `DeviceGroupRuntime`、`ControlRuntime` 已支持一对一 glass-phone 绑定、自动绑定和指定目标绑定。 | 业务只表达“哪个眼镜与哪个手机绑定”的产品关系，不处理控制通道细节。 |
...
| 生产级多用户、多眼镜、多手机组织管理 | 第 1/9 项产品化扩展 | 当前 SDK 主要解决设备运行时绑定，不提供账号体系、设备目录、授权和审计。 | 功能团队不应在业务 Task 中自行实现账号或组织模型。 |
...
7. 遇到实时语音、公网直连、复杂抢播、多设备组织、生产级持久化等问题时，不要在业务层临时补系统能力，应写入“架构阻塞点说明与改进建议”。
