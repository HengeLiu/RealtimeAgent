...
`app-examples/for-blind-app` 是当前唯一推荐给业务开发者启动和扩展的完整示例应用。它把旧 SDK 的找物、红绿灯、导航、搜索、计时器，以及 SDK 级抓拍、设备状态、连续视觉 stream、Task、MCP wrapper、Text Agent 和 Realtime Agent 验收入口都放在同一个 app-root 中。

| 目录 | 当前定位 |
| --- | --- |
| `for-blind-app` | 唯一应用示例。用于真实 server 启动、Tool / Task 自动发现、旧 SDK 能力迁移、SDK 基础能力和设备级回放。 |
