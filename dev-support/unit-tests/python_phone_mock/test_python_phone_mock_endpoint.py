...
def test_endpoint_reference_directories_exist() -> None:
    ...
    assert (root / "dev-support/devices/browser-glass/README.md").exists()
    assert (root / "dev-support/devices/python-phone/phone.mock.yaml").exists()
    assert (root / "examples/device_app_demo/ios/DeviceDemo.xcodeproj/project.pbxproj").exists()
    assert (root / "examples/simple-agent-server/server.yaml").exists()
...
def test_python_phone_reference_uses_python_device_sdk() -> None:
    ...
    root = Path(__file__).resolve().parents[3]
    source_files = [
        root / "dev-support/devices/python-glass/realtime_agent_python_glass/playback.py",
        root / "dev-support/devices/python-phone/realtime_agent_python_phone_mock/phone_mock.py",
        root / "dev-support/devices/python-phone/realtime_agent_python_phone_mock/playback_fallback.py",
        root / "dev-support/devices/python-phone/realtime_agent_python_phone_mock/remote_task.py",
    ]
