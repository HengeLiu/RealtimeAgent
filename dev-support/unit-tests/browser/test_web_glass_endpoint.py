...
def _html() -> str:
    return Path(__file__).resolve().parents[3].joinpath("dev-support/devices/browser-glass/index.html").read_text(encoding="utf-8")


def _browser_sdk_adapter() -> str:
    return Path(__file__).resolve().parents[3].joinpath(
        "dev-support/devices/browser-glass/sdk/realtime-agent-device-browser.js"
...
def test_web_glass_stream_chunk_codec_shape_is_protocol_compatible() -> None:
    ...
    assert 'export * from "../../../../devices/javascript/src/index.js";' in sdk_adapter
...
def test_web_glass_is_not_served_by_sdk_server() -> None:
    ...
    """
    server_source = Path(__file__).resolve().parents[3].joinpath("agent-server/realtime_agent/server.py").read_text(
        encoding="utf-8"
    )
