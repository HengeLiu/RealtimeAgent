...
文档状态：下一阶段设计稿。本文记录新的定位：系统测试能力属于 `dev-support` 下的特殊端侧实现，名称暂定为 `python-playback-glass`，即“眼镜回放设备”。它对 server 必须像普通端侧一样透明，不能成为 SDK 内部测试框架。
  ...
  "cases": [
    {
      ...
      "runs_dir": "examples/simple-agent-server/runs/user-system-test/dev-python-playback-glass"
    }
  ]
}
```
