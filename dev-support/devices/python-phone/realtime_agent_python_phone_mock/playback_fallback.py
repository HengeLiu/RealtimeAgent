...
from realtime_agent_device import RealtimeAgentDeviceClient, RealtimeAgentEvent as Event, StreamChunk, StreamChunkCodec, new_id, ws_url
...
class NetworkPythonPlaybackEndpoint:
    ...
    主要功能：当当前 worktree 缺少 `realtime_agent_python_glass.playback` 时，为
            ...
            if event.event_name == "stream.control.open.requested" and event.stream_type == "sensor.rgb":
                ...
                await self._send_event(
                    ...
                    Event(
                        ...
                        stream_id=event.stream_id,
                        stream_type=event.stream_type,
                        payload={"stream_type": event.stream_type, "reason": "fallback_finished"},
                    ),
                )
