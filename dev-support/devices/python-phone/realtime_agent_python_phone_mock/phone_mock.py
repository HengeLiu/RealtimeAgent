...
from realtime_agent_device import RealtimeAgentEvent as Event, StreamChunk, StreamChunkCodec, new_id
...
try:
    ...
    if exc.name != "realtime_agent_python_glass":
        raise
...
def _resolve_output_path(raw_path: str | Path) -> Path:
    ...
    CLI 时把最近一帧写入 `runs/realtime-agent/...`。
    参数：`raw_path` 为配置路径。
    返回值：解析后的绝对路径。
    异常情况：无。
    """
...
class OpenCvVideoPreview:
    ...
    def __init__(self, *, cv2_module: Any, enabled: bool = True, window_title: str = "realtime-agent python phone", max_fps: float = 15.0) -> None:
...
class NetworkPythonPhoneMockEndpoint(NetworkPythonPlaybackEndpoint):
        ...
        if display_enabled or save_latest_frame:
            ...
            if display_backend in {"opencv", "cv2"}:
                self.video_preview = OpenCvVideoPreview(
                    ...
                    window_title=str(display_config.get("window_title") or "realtime-agent python phone"),
    ...
    async def _control_loop(self, control_ws, stream_ws, audio_payload: bytes | None) -> None:
        ...
        async for message in control_ws:
            ...
            elif event.event_name == "stream.output.start.requested" and str(event.stream_type or "").startswith("actuator."):
                await self._send_event(
                    control_ws,
                    Event(
                        ...
                        stream_type=event.stream_type,
                        payload={"stream_type": event.stream_type, "reason": "phone_mock_ready"},
                    ),
                )
            elif event.event_name == "command.requested":
                await self._handle_device_command(control_ws, stream_ws, event)
            elif event.event_name in {"stream.output.close.requested", "stream.output.finish.requested"}:
                await self._send_event(
                    control_ws,
                    Event(
...
def run_network_phone_preview_gui(config: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    window = PhonePreviewWindow(
        bridge=bridge,
        title=str(display_config.get("window_title") or "realtime-agent Python Phone"),
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser = argparse.ArgumentParser(prog="python -m realtime_agent_python_phone_mock", description="启动 realtime-agent Python phone mock")
