...
from realtime_agent_device import RealtimeAgentDeviceClient, RealtimeAgentEvent as Event, StreamChunk as DeviceStreamChunk, ws_url
    ...
    def __init__(
        ...
        *,
        ...
        if properties is None:
            ...
            if "speaker.role" in self.properties and "realtime_agent.audio_output" not in self.properties:
                self.properties["realtime_agent.audio_output"] = "actuator.speaker"
            if "microphone.role" in self.properties and "realtime_agent.audio_input" not in self.properties:
                self.properties["realtime_agent.audio_input"] = "sensor.mic"
        ...
        self.client = RealtimeAgentDeviceClient(
            ...
            device={
                ...
                "sdk_version": "realtime-agent-python-glass-compat-0.1.0",
                "auth": self.auth,
    ...
    async def _control_loop(self, control_ws, stream_ws, audio: PlaybackAudio | None) -> None:
        ...
        while not control_ws.closed:
            ...
            if await self.client.dispatch_event(event):
                continue
            if event.event_name == "stream.output.start.requested" and event.stream_type == "actuator.speaker":
...
class PythonPlaybackEndpoint:
    ...
    `RealtimeAgentApp`，并按端侧行为响应 RGB 采集和输出 stream。
    ...
    def push_event(self, event: Event) -> None:
        ...
        elif event.event_name == "stream.output.start.requested" and event.stream_type == "actuator.speaker":
    ...
    def _register(self) -> None:
        """向 `RealtimeAgentApp` 注册具备麦克风、扬声器和 RGB 的测试设备。"""
...
def run_playback(config: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    主要逻辑：构造 `RealtimeAgentApp` 和 `PythonPlaybackEndpoint`，用真实 WAV 或默认样例
    ...
    data = dict(config or {})
    app = RealtimeAgentApp(RealtimeAgentConfig(runs_root=str(data.get("runs_root") or "runs/realtime-agent"), agent_mode="vision"))
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(prog="realtime-agent.playback.glass", description="运行 realtime-agent Python glass playback 参考端")
    parser.parse_args()
